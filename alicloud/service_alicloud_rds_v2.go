...
// DescribeRdsBackupFile <<< Encapsulated get interface for Rds BackupFile.

func (s *RdsServiceV2) DescribeRdsBackupFile(id string) (object map[string]interface{}, err error) {
	client := s.client
	var request map[string]interface{}
	var response map[string]interface{}
	var query map[string]interface{}
	...
	if len(v.([]interface{})) == 0 {
		return object, WrapErrorf(NotFoundErr("BackupFile", id), NotFoundMsg, response)
	}

	return v.([]interface{})[0].(map[string]interface{}), nil
}

func (s *RdsServiceV2) RdsBackupFileStateRefreshFunc(id string, field string, failStates []string) resource.StateRefreshFunc {
	return s.RdsBackupFileStateRefreshFuncWithApi(id, field, failStates, s.DescribeRdsBackupFile)
}

func (s *RdsServiceV2) RdsBackupFileStateRefreshFuncWithApi(id string, field string, failStates []string, call func(id string) (map[string]interface{}, error)) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		object, err := call(id)
		if err != nil {
			if NotFoundError(err) {
...
// DescribeRdsBackupFile >>> Encapsulated.
