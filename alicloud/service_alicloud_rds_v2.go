...
// DescribeRdsGadInstance <<< Encapsulated get interface for Rds GadInstance.

func (s *RdsServiceV2) DescribeRdsGadInstance(id string) (object map[string]interface{}, err error) {
	client := s.client
	var request map[string]interface{}
	var response map[string]interface{}
	var query map[string]interface{}
	...
	v, err := jsonpath.Get("$.GadInstances[*]", response)
	if err != nil {
		return object, WrapErrorf(err, FailedGetAttributeMsg, id, "$.GadInstances[*]", response)
	}

	if len(v.([]interface{})) == 0 {
		return object, WrapErrorf(NotFoundErr("GadInstance", id), NotFoundMsg, response)
	}

	return v.([]interface{})[0].(map[string]interface{}), nil
}

func (s *RdsServiceV2) RdsGadInstanceStateRefreshFunc(id string, field string, failStates []string) resource.StateRefreshFunc {
	return s.RdsGadInstanceStateRefreshFuncWithApi(id, field, failStates, s.DescribeRdsGadInstance)
}

func (s *RdsServiceV2) RdsGadInstanceStateRefreshFuncWithApi(id string, field string, failStates []string, call func(id string) (map[string]interface{}, error)) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		object, err := call(id)
		if err != nil {
			if NotFoundError(err) {
...
// DescribeRdsGadInstance >>> Encapsulated.
