...
// DescribePolardbLakeCache <<< Encapsulated get interface for Polardb LakeCache.

func (s *PolardbServiceV2) DescribePolardbLakeCache(id string) (object map[string]interface{}, err error) {
	client := s.client
	var request map[string]interface{}
	var response map[string]interface{}
	var query map[string]interface{}
	...
	addDebug(action, response, request)
	if err != nil {
		if IsExpectedErrors(err, []string{"InvalidPolarFsInstanceId.NotFound"}) {
			return object, WrapErrorf(NotFoundErr("LakeCache", id), NotFoundMsg, response)
		}
		return object, WrapErrorf(err, DefaultErrorMsg, id, action, AlibabaCloudSdkGoERROR)
	}
...
func (s *PolardbServiceV2) DescribeLakeCacheListTagResources(id string) (object map[string]interface{}, err error) {
	client := s.client
	var request map[string]interface{}
	var response map[string]interface{}
	var query map[string]interface{}
...
func (s *PolardbServiceV2) PolardbLakeCacheStateRefreshFunc(id string, field string, failStates []string) resource.StateRefreshFunc {
	return s.PolardbLakeCacheStateRefreshFuncWithApi(id, field, failStates, s.DescribePolardbLakeCache)
}

func (s *PolardbServiceV2) PolardbLakeCacheStateRefreshFuncWithApi(id string, field string, failStates []string, call func(id string) (map[string]interface{}, error)) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		object, err := call(id)
		if err != nil {
			if NotFoundError(err) {
...
// DescribePolardbLakeCache >>> Encapsulated.
