...
// DescribePolardbZonalEndpoint <<< Encapsulated get interface for Polardb ZonalEndpoint.

func (s *PolardbServiceV2) DescribePolardbZonalEndpoint(id string) (object map[string]interface{}, err error) {
	client := s.client
	var request map[string]interface{}
	var response map[string]interface{}
	var query map[string]interface{}
	...
	if err != nil {
		if IsExpectedErrors(err, []string{"InvalidDBClusterId.NotFound"}) {
			return object, WrapErrorf(NotFoundErr("ZonalEndpoint", id), NotFoundMsg, response)
		}
		return object, WrapErrorf(err, DefaultErrorMsg, id, action, AlibabaCloudSdkGoERROR)
	}
...
func (s *PolardbServiceV2) PolardbZonalEndpointStateRefreshFunc(id string, field string, failStates []string) resource.StateRefreshFunc {
	return s.PolardbZonalEndpointStateRefreshFuncWithApi(id, field, failStates, s.DescribePolardbZonalEndpoint)
}

func (s *PolardbServiceV2) PolardbZonalEndpointStateRefreshFuncWithApi(id string, field string, failStates []string, call func(id string) (map[string]interface{}, error)) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		object, err := call(id)
		if err != nil {
			if NotFoundError(err) {
...
// DescribePolardbZonalEndpoint >>> Encapsulated.
