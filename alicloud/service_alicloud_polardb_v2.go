...
// DescribePolardbApplication <<< Encapsulated get interface for Polardb Application.

func (s *PolardbServiceV2) DescribePolardbApplication(id string) (object map[string]interface{}, err error) {
	client := s.client
	var request map[string]interface{}
	var response map[string]interface{}
	var query map[string]interface{}
	...
	addDebug(action, response, request)
	if err != nil {
		if IsExpectedErrors(err, []string{"InvalidApplicationId.NotFound"}) {
			return object, WrapErrorf(NotFoundErr("Application", id), NotFoundMsg, response)
		}
		return object, WrapErrorf(err, DefaultErrorMsg, id, action, AlibabaCloudSdkGoERROR)
	}
...
func (s *PolardbServiceV2) DescribeApplicationListTagResources(id string) (object map[string]interface{}, err error) {
	client := s.client
	var request map[string]interface{}
	var response map[string]interface{}
	var query map[string]interface{}
...
func (s *PolardbServiceV2) PolardbApplicationStateRefreshFunc(id string, field string, failStates []string) resource.StateRefreshFunc {
	return s.PolardbApplicationStateRefreshFuncWithApi(id, field, failStates, s.DescribePolardbApplication)
}

func (s *PolardbServiceV2) PolardbApplicationStateRefreshFuncWithApi(id string, field string, failStates []string, call func(id string) (map[string]interface{}, error)) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		object, err := call(id)
		if err != nil {
			if NotFoundError(err) {
		...
		object["PayType"] = convertPolardbApplicationPayTypeResponse(object["PayType"])
		v, err := jsonpath.Get(field, object)
		currentStatus := fmt.Sprint(v)
...
// DescribePolardbApplication >>> Encapsulated.
