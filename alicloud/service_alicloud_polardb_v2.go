...
// DescribePolardbZonalDatabase <<< Encapsulated get interface for Polardb ZonalDatabase.

func (s *PolardbServiceV2) DescribePolardbZonalDatabase(id string) (object map[string]interface{}, err error) {
	client := s.client
	var request map[string]interface{}
	var response map[string]interface{}
	var query map[string]interface{}
	...
	if len(v.([]interface{})) == 0 {
		return object, WrapErrorf(NotFoundErr("ZonalDatabase", id), NotFoundMsg, response)
	}

	result, _ := v.([]interface{})
	for _, v := range result {
		item := v.(map[string]interface{})
		if fmt.Sprint(item["DBName"]) != parts[1] {
			continue
		}
		return item, nil
	}
...
func (s *PolardbServiceV2) PolardbZonalDatabaseStateRefreshFunc(id string, field string, failStates []string) resource.StateRefreshFunc {
	return s.PolardbZonalDatabaseStateRefreshFuncWithApi(id, field, failStates, s.DescribePolardbZonalDatabase)
}

func (s *PolardbServiceV2) PolardbZonalDatabaseStateRefreshFuncWithApi(id string, field string, failStates []string, call func(id string) (map[string]interface{}, error)) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		object, err := call(id)
		if err != nil {
			if NotFoundError(err) {
...
// DescribePolardbZonalDatabase >>> Encapsulated.
