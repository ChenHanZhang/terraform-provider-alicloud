...
// DescribePolardbAICluster <<< Encapsulated get interface for Polardb AICluster.

func (s *PolardbServiceV2) DescribePolardbAICluster(id string) (object map[string]interface{}, err error) {
	client := s.client
	var request map[string]interface{}
	var response map[string]interface{}
	var query map[string]interface{}
	...
	if err != nil {
		if IsExpectedErrors(err, []string{"InvalidDBClusterId.NotFound"}) {
			return object, WrapErrorf(NotFoundErr("AICluster", id), NotFoundMsg, response)
		}
		return object, WrapErrorf(err, DefaultErrorMsg, id, action, AlibabaCloudSdkGoERROR)
	}
...
func (s *PolardbServiceV2) DescribeAIClusterListTagResources(id string) (object map[string]interface{}, err error) {
	client := s.client
	var request map[string]interface{}
	var response map[string]interface{}
	var query map[string]interface{}
...
func (s *PolardbServiceV2) PolardbAIClusterStateRefreshFunc(id string, field string, failStates []string) resource.StateRefreshFunc {
	return s.PolardbAIClusterStateRefreshFuncWithApi(id, field, failStates, s.DescribePolardbAICluster)
}

func (s *PolardbServiceV2) PolardbAIClusterStateRefreshFuncWithApi(id string, field string, failStates []string, call func(id string) (map[string]interface{}, error)) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		object, err := call(id)
		if err != nil {
			if NotFoundError(err) {
...
// DescribePolardbAICluster >>> Encapsulated.
