		...
		Steps: []resource.TestStep{
			{
				Config: testAccConfig(map[string]interface{}{
					...
					"centralize_config": []map[string]interface{}{
						{
							"dest_ttl":      "3",
							"dest_region":   "${alicloud_log_project.all_project01.region_id}",
							"dest_project":  "${alicloud_log_project.all_project01.id}",
			...
			},
			{
				Config: testAccConfig(map[string]interface{}{
					...
					"centralize_config": []map[string]interface{}{
						{
							"dest_region":   "${alicloud_log_project.all_project02.region_id}",
							"dest_project":  "${alicloud_log_project.all_project02.id}",
...
resource "alicloud_log_project" "all_project01" {
  description = "xccx"
  name        = "tf-all-project-01-435"
}

resource "alicloud_log_store" "all_logstore_01" {
  retention_period = "30"
  shard_count      = "2"
  project          = alicloud_log_project.all_project01.id
  name             = "tf-all-logstore-01-840"
}

resource "alicloud_log_project" "all_project02" {
  description = "cvcvc"
  name        = "tf-all-project-02-442"
}

resource "alicloud_log_store" "all_logstore002" {
  retention_period = "30"
  shard_count      = "2"
  project          = alicloud_log_project.all_project02.id
  name             = "tf-all-logstore-02-459"
}
