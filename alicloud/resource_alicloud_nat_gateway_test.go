	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				ImportState:             true,
				ImportStateVerify:       true,
				ImportStateVerifyIgnore: []string{"access_mode", "log_delivery", "private_link_enabled", "security_protection_enabled"},
			},
		},
	})
}
...
resource "alicloud_vpc" "vpc" {
  ...
  cidr_block  = "172.16.0.0/12"
  vpc_name    = "tf-test-nat-session-log-vpc"
}
...
// Case 全生命周期_对接TF 4060
func TestAccAliCloudNATGatewayNatGateway_basic4060(t *testing.T) {
	var v map[string]interface{}
	resourceId := "alicloud_nat_gateway.default"
	ra := resourceAttrInit(resourceId, AlicloudNATGatewayNatGatewayMap4060)
	rc := resourceCheckInitWithDescribeMethod(resourceId, &v, func() interface{} {
		return &NATGatewayServiceV2{testAccProvider.Meta().(*connectivity.AliyunClient)}
	}, "DescribeNATGatewayNatGateway")
	rac := resourceAttrCheckInit(rc, ra)
	...
	rand := acctest.RandIntRange(10000, 99999)
	name := fmt.Sprintf("tfaccnatgateway%d", rand)
	testAccConfig := resourceTestAccConfigFunc(resourceId, name, AlicloudNATGatewayNatGatewayBasicDependence4060)
	resource.Test(t, resource.TestCase{
		PreCheck: func() {
...
var AlicloudNATGatewayNatGatewayMap4060 = map[string]string{
	"status":              CHECKSET,
	"create_time":         CHECKSET,
	"forward_table_ids.#": CHECKSET,
	"deletion_protection": CHECKSET,
...
func AlicloudNATGatewayNatGatewayBasicDependence4060(name string) string {
	return fmt.Sprintf(`
variable "name" {
    default = "%s"
}
