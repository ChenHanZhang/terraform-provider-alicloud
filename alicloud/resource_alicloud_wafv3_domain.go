...
func resourceAliCloudWafv3DomainCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if v := d.Get("redirect"); v != nil {
		...
		if maxBodySize1 != nil && maxBodySize1 != "" {
		...
		if http2OriginMaxConcurrency1 != nil && http2OriginMaxConcurrency1 != "" {
...
func resourceAliCloudWafv3DomainUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	query = make(map[string]interface{})
	request["RegionId"] = client.RegionId
	if !d.IsNewResource() && d.HasChange("listen.0.enable_tlsv3") {
		update = true
	}
	listenEnableTLSv3JsonPath, err := jsonpath.Get("$[0].enable_tlsv3", d.Get("listen"))
	if err == nil {
		request["EnableTLSv3"] = listenEnableTLSv3JsonPath
	}

	if !d.IsNewResource() && d.HasChange("listen.0.cipher_suite") {
		update = true
	}
	listenCipherSuiteJsonPath, err := jsonpath.Get("$[0].cipher_suite", d.Get("listen"))
	if err == nil {
		request["CipherSuite"] = listenCipherSuiteJsonPath
	}

	if !d.IsNewResource() && d.HasChange("listen.0.cert_id") {
		update = true
	}
	listenCertIdJsonPath, err := jsonpath.Get("$[0].cert_id", d.Get("listen"))
	if err == nil {
		request["CertId"] = listenCertIdJsonPath
	}

	if !d.IsNewResource() && d.HasChange("listen.0.tls_version") {
		update = true
	}
	listenTLSVersionJsonPath, err := jsonpath.Get("$[0].tls_version", d.Get("listen"))
	if err == nil {
		request["TLSVersion"] = listenTLSVersionJsonPath
	}
