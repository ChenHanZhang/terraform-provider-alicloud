	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"spec": {
				...
				Required:     true,
				ForceNew:     true,
				ValidateFunc: StringInSlice([]string{"premium_version", "payg_version", "enterprise_version", "ultimate_version"}, false),
			},
			"status": {
				Type:     schema.TypeString,
				Computed: true,
...
func resourceAliCloudCloudFirewallInstanceRead(d *schema.ResourceData, meta interface{}) error {
	...
	d.Set("fw_vpc_number", objectRaw["VpcNumber"])
	d.Set("ip_number", objectRaw["IpNumber"])
	d.Set("sdl", formatBool(convertCloudFirewallInstanceSdlResponse(objectRaw["Sdl"])))
