...
func resourceAliCloudCloudFirewallInstanceCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := d.GetOk("cfw_ndlp_enable"); ok {
		parameterMapList = append(parameterMapList, map[string]interface{}{
			"Code":  "cfw_ndlp_enable",
			"Value": fmt.Sprint(v),
		})
	}
	if v, ok := d.GetOk("cfw_bandwidth"); ok {
		parameterMapList = append(parameterMapList, map[string]interface{}{
			"Code":  "cfw_bandwidth",
			"Value": fmt.Sprint(v),
		})
	...
	if v, ok := d.GetOk("cfw_elastic_traffic"); ok {
		parameterMapList = append(parameterMapList, map[string]interface{}{
			"Code":  "cfw_elastictraffic",
			"Value": fmt.Sprint(v),
		})
	}
	...
	if v, ok := d.GetOk("cfw_endpoint"); ok {
		parameterMapList = append(parameterMapList, map[string]interface{}{
			"Code":  "cfw_endpoint",
			"Value": fmt.Sprint(v),
		})
...
func resourceAliCloudCloudFirewallInstanceRead(d *schema.ResourceData, meta interface{}) error {
	...
	d.Set("cfw_bandwidth", objectRaw["ExtensionBandwidth"])
	...
	e := jsonata.MustCompile("$.LogStorage / 1024")
	evaluation, _ := e.Eval(objectRaw)
	d.Set("cfw_log_storage", evaluation)
	e = jsonata.MustCompile("$.Sdl ? true : false\n")
	evaluation, _ = e.Eval(objectRaw)
	d.Set("sdl", evaluation)
	objectRaw, err = cloudFirewallServiceV2.DescribeInstanceDescribeAssetStatistic(d.Id())
	if err != nil && !NotFoundError(err) {
		return WrapError(err)
	}

	e := jsonata.MustCompile("$.AutoResourceEnable ? \"true\" : \"false\"")
	evaluation, _ := e.Eval(objectRaw)
	d.Set("auto_asset_protection", evaluation)
...
func resourceAliCloudCloudFirewallInstanceUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	request["ProductCode"] = d.Get("product_code")
	if !d.IsNewResource() && d.HasChanges("cfw_ndlp_enable", "cfw_elastic_traffic", "cfw_log_storage", "cfw_bandwidth", "cfw_endpoint", "cfw_log") {
		update = true
	}
	parameterMapList := make([]map[string]interface{}, 0)
	if v, ok := d.GetOk("cfw_ndlp_enable"); ok {
...
func resourceAliCloudCloudFirewallInstanceDelete(d *schema.ResourceData, meta interface{}) error {
	...
	if enableDelete {
		...
		request["ProductCode"] = d.Get("product_code")
		request["ProductType"] = d.Get("product_type")
		if v, ok := d.GetOkExists("immediately_release"); ok {
			request["ImmediatelyRelease"] = v
		}
		var endpoint string
		request["ProductCode"] = ""
		request["ProductType"] = ""
