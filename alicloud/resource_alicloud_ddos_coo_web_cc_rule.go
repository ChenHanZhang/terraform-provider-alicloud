...
request["Domain"] = v
        ...
        if countThreshold != nil                && countThreshold != "" {
        ...
        if ratioThreshold != nil                && ratioThreshold != "" {
...
for _, dataLoop := range convertToInterfaceArray(localData) {
    ...
    RuleListMap = append(RuleListMap, ruleListDataList)
            request["RuleList"] = RuleListMap
    
                                                            request["Expires"] = "0"
                                                                jsonString := convertObjectToJsonString(request)
                    jsonString, _ = sjson.Set(jsonString, "RuleList.0.name", d.Get("name"))
    ...
    if err != nil {
        return WrapErrorf(err, DefaultErrorMsg, "alicloud_ddos_coo_web_cc_rule", action, AlibabaCloudSdkGoERROR)
    }
        
    
                                                        RuleListnameVar, _ := jsonpath.Get("RuleList[0].name", request)
                            d.SetId(fmt.Sprintf("%v:%v",request["Domain"],RuleListnameVar));
		...
		ddosCooServiceV2 := DdosCooServiceV2{client}
				...
				log.Printf("[DEBUG] Resource alicloud_ddos_coo_web_cc_rule DescribeDdosCooWebCcRule Failed!!! %s", err)
				d.SetId("")
				return nil
			}
...
if len(ruleDetailRaw) > 0 {
                                        ruleDetailMap["action"] = ruleDetailRaw["Action"]
                                                                                                                                                                            ...
                                                                                                                                                                            conditionChildRaw := conditionChildRaw.(map[string]interface{})
...
for _, dataLoop := range convertToInterfaceArray(localData) {
    ...
    RuleListMap = append(RuleListMap, ruleListDataList)
            request["RuleList"] = RuleListMap
                                                                    ...
                                                                    request["Expires"] = "0"
                                                                                        jsonString := convertObjectToJsonString(request)
                    jsonString, _ = sjson.Set(jsonString, "RuleList.0.name", parts[1])
                _ = json.Unmarshal([]byte(jsonString), &request)
