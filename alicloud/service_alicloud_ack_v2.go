...
// DescribeAckInspectConfig <<< Encapsulated get interface for Ack InspectConfig.

func (s *AckServiceV2) DescribeAckInspectConfig(id string) (object map[string]interface{}, err error) {
	client := s.client
	var request map[string]interface{}
	var response map[string]interface{}
	var query map[string]*string
	...
	addDebug(action, response, request)
	if err != nil {
		if IsExpectedErrors(err, []string{"InspectConfig.NotFound"}) {
			return object, WrapErrorf(NotFoundErr("InspectConfig", id), NotFoundMsg, response)
		}
		return object, WrapErrorf(err, DefaultErrorMsg, id, action, AlibabaCloudSdkGoERROR)
	}
...
func (s *AckServiceV2) AckInspectConfigStateRefreshFunc(id string, field string, failStates []string) resource.StateRefreshFunc {
	return s.AckInspectConfigStateRefreshFuncWithApi(id, field, failStates, s.DescribeAckInspectConfig)
}

func (s *AckServiceV2) AckInspectConfigStateRefreshFuncWithApi(id string, field string, failStates []string, call func(id string) (map[string]interface{}, error)) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		object, err := call(id)
		if err != nil {
			if NotFoundError(err) {
...
// DescribeAckInspectConfig >>> Encapsulated.
