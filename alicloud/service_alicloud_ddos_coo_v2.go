...
func (s *DdosCooServiceV2) DescribeDdosCooDomainPreciseAccessRule(id string) (object map[string]interface{}, err error) {
	...
	v, err := jsonpath.Get("$.PreciseAccessConfigList[*]", response)
	if err != nil {
	...
	for _, v := range result {
		...
		names, err := jsonpath.Get("$.RuleList[*]", item)
		if err != nil {
			return object, WrapErrorf(err, FailedGetAttributeMsg, id, "$.RuleList[*]", item)
		}
		found := false
		for _, vv := range names.([]interface{}) {
			if vv.(map[string]interface{})["Name"] == parts[1] {
				found = true
				break
			}
		}
		...
		owner, err := jsonpath.Get("$.RuleList[*].Owner", item)
		if err != nil {
			return object, WrapErrorf(err, FailedGetAttributeMsg, id, "$.RuleList[*].Owner", item)
		}
		if owner != "manual" {
			continue
		}
