...
func resourceAliCloudEsaRatePlanInstanceCreate(d *schema.ResourceData, meta interface{}) error {
	...
	return resourceAliCloudEsaRatePlanInstanceUpdate(d, meta)
}
...
func resourceAliCloudEsaRatePlanInstanceRead(d *schema.ResourceData, meta interface{}) error {
	...
	objectRaw, err = esaServiceV2.DescribeRatePlanInstanceDescribeDdosMaxBurstGbps(d.Id())
	if err != nil && !NotFoundError(err) {
		return WrapError(err)
	}

	d.Set("max_burst_gbps", objectRaw["MaxBurstGbps"])
...
func resourceAliCloudEsaRatePlanInstanceUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChange("max_burst_gbps") {
		update = true
	}
	request["MaxBurstGbps"] = d.Get("max_burst_gbps")
	if update {
		wait := incrementalWait(3*time.Second, 5*time.Second)
		err = resource.Retry(d.Timeout(schema.TimeoutUpdate), func() *resource.RetryError {
			response, err = client.RpcPost("ESA", "2024-09-10", action, query, request, true)
