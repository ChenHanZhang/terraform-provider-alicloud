...
func resourceAliCloudEhpcClusterV2Create(d *schema.ResourceData, meta interface{}) error {
	...
	stateConf := BuildStateConf([]string{}, []string{"running"}, d.Timeout(schema.TimeoutCreate), 30*time.Second, ehpcServiceV2.EhpcClusterV2StateRefreshFuncWithApi(d.Id(), "$.ClusterStatus", []string{"exception"}, ehpcServiceV2.DescribeClusterV2GetCluster))
...
func resourceAliCloudEhpcClusterV2Read(d *schema.ResourceData, meta interface{}) error {
	...
	mountInfoRawObj, _ := jsonpath.Get("$.MountInfo[*]", objectRaw)
	mountInfoRaw := make([]interface{}, 0)
	if mountInfoRawObj != nil {
		mountInfoRaw = convertToInterfaceArray(mountInfoRawObj)
	}

	mountInfoChildRaw := mountInfoChildRaw.(map[string]interface{})
	sharedStoragesMaps := make([]map[string]interface{}, 0)
	if mountInfoChildRaw != nil {
		for _, mountDirectoryRaw := range convertToInterfaceArray(mountInfoChildRaw) {
			sharedStoragesMap := make(map[string]interface{})
			mountInfoChildRaw := mountInfoChildRaw.(map[string]interface{})
			sharedStoragesMap["mount_directory"] = mountInfoChildRaw["MountDirectory"]
			sharedStoragesMap["mount_options"] = mountInfoChildRaw["MountOptions"]
			sharedStoragesMap["mount_target_domain"] = mountInfoChildRaw["MountTarget"]
			sharedStoragesMap["protocol_type"] = mountInfoChildRaw["ProtocolType"]
	...
	d.Set("deletion_protection", formatBool(objectRaw["DeleteProtection"]))
