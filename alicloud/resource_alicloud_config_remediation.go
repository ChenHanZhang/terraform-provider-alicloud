...
func resourceAlicloudConfigRemediationCreate(d *schema.ResourceData, meta interface{}) error {
	...
	return resourceAlicloudConfigRemediationUpdate(d, meta)
}
...
func resourceAlicloudConfigRemediationRead(d *schema.ResourceData, meta interface{}) error {
	...
	object, err := configServiceV2.DescribeConfigRemediation(d.Id())
	if err != nil {
	...
	d.Set("invoke_type", object["invoke_type"])
	d.Set("params", object["params"])
	d.Set("remediation_id", object["remediation_id"])
	d.Set("remediation_source_type", object["remediation_source_type"])
	d.Set("remediation_template_id", object["remediation_template_id"])
	d.Set("remediation_type", object["remediation_type"])
...
func resourceAlicloudConfigRemediationUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if !d.IsNewResource() && d.HasChange("params") {
		update = true
		if v, ok := d.GetOk("params"); ok {
			request["Params"] = v
		}
	}
	if !d.IsNewResource() && d.HasChange("invoke_type") {
		update = true
		if v, ok := d.GetOk("invoke_type"); ok {
			request["InvokeType"] = v
		}
	}
	if !d.IsNewResource() && d.HasChange("remediation_template_id") {
		update = true
		if v, ok := d.GetOk("remediation_template_id"); ok {
			request["RemediationTemplateId"] = v
		}
