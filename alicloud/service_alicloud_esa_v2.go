...
func (s *EsaServiceV2) DescribeEsaHttpIncomingResponseHeaderModificationRule(id string) (object map[string]interface{}, err error) {
	// AUTOFIX: Add retry mechanism for DescribeEsaHttpIncomingResponseHeaderModificationRule
	retry := &common.RetryPolicy{
		Timeout: timeout,
		Interval: 10 * time.Second,
	}
	return common.Retry(func() (interface{}, error) {
		networkResp, err := s.client.WithEsaClient(func(esaClient *esa.Client) (interface{}, error) {
			return esaClient.GetHttpIncomingResponseHeaderModificationRule(&esa.GetHttpIncomingResponseHeaderModificationRuleRequest{
				Id: id,
			})
		})
		if err != nil {
		...
		resp := networkResp.(*esa.GetHttpIncomingResponseHeaderModificationRuleResponse)
		return resp, nil
	}, retry)
	client := s.client
	var request map[string]interface{}
