...
// DescribeCloudMonitorServiceMonitoringAgentProcess <<< Encapsulated get interface for CloudMonitorService MonitoringAgentProcess.
...
func (s *CloudMonitorServiceServiceV2) DescribeCloudMonitorServiceMonitoringAgentProcess(id string) (object map[string]interface{}, err error) {
	...
	v, err := jsonpath.Get("$.NodeProcesses.NodeProcess[*]", response)
	if err != nil {
		return object, WrapErrorf(err, FailedGetAttributeMsg, id, "$.NodeProcesses.NodeProcess[*]", response)
	}

	if len(v.([]interface{})) == 0 {
		return object, WrapErrorf(NotFoundErr("MonitoringAgentProcess", id), NotFoundMsg, response)
	}

	result, _ := v.([]interface{})
	for _, v := range result {
		item := v.(map[string]interface{})
		if fmt.Sprint(item["ProcessId"]) != parts[1] {
			continue
		}
		return item, nil
	}
...
func (s *CloudMonitorServiceServiceV2) CloudMonitorServiceMonitoringAgentProcessStateRefreshFunc(id string, field string, failStates []string) resource.StateRefreshFunc {
	return s.CloudMonitorServiceMonitoringAgentProcessStateRefreshFuncWithApi(id, field, failStates, s.DescribeCloudMonitorServiceMonitoringAgentProcess)
}

func (s *CloudMonitorServiceServiceV2) CloudMonitorServiceMonitoringAgentProcessStateRefreshFuncWithApi(id string, field string, failStates []string, call func(id string) (map[string]interface{}, error)) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		object, err := call(id)
		if err != nil {
			if NotFoundError(err) {
...
// DescribeCloudMonitorServiceMonitoringAgentProcess >>> Encapsulated.
