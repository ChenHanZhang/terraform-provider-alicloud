...
// DescribeCloudMonitorServiceAlarmContactGroup <<< Encapsulated get interface for CloudMonitorService AlarmContactGroup.

func (s *CloudMonitorServiceServiceV2) DescribeCloudMonitorServiceAlarmContactGroup(id string) (object map[string]interface{}, err error) {
	client := s.client
	var request map[string]interface{}
	var response map[string]interface{}
	var query map[string]interface{}
	...
	addDebug(action, response, request)
	if err != nil {
		if IsExpectedErrors(err, []string{"ContactGroupNotExists"}) {
			return object, WrapErrorf(NotFoundErr("AlarmContactGroup", id), NotFoundMsg, response)
		}
		return object, WrapErrorf(err, DefaultErrorMsg, id, action, AlibabaCloudSdkGoERROR)
	}

	v, err := jsonpath.Get("$.ContactGroupList.ContactGroup[*]", response)
	if err != nil {
		return object, WrapErrorf(err, FailedGetAttributeMsg, id, "$.ContactGroupList.ContactGroup[*]", response)
	}
...
func (s *CloudMonitorServiceServiceV2) CloudMonitorServiceAlarmContactGroupStateRefreshFunc(id string, field string, failStates []string) resource.StateRefreshFunc {
	return s.CloudMonitorServiceAlarmContactGroupStateRefreshFuncWithApi(id, field, failStates, s.DescribeCloudMonitorServiceAlarmContactGroup)
}

func (s *CloudMonitorServiceServiceV2) CloudMonitorServiceAlarmContactGroupStateRefreshFuncWithApi(id string, field string, failStates []string, call func(id string) (map[string]interface{}, error)) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		object, err := call(id)
		if err != nil {
			if NotFoundError(err) {
...
// DescribeCloudMonitorServiceAlarmContactGroup >>> Encapsulated.
