...
func resourceAliCloudAliKafkaAclCreate(d *schema.ResourceData, meta interface{}) error {
	...
	d.SetId(fmt.Sprintf("%v:%v:%v:%v:%v:%v", request["InstanceId"], request["Username"], request["AclResourceName"], request["AclResourceType"], request["AclOperationType"], request["AclResourcePatternType"]))
...
func resourceAliCloudAliKafkaAclDelete(d *schema.ResourceData, meta interface{}) error {
	...
	var err error
	request = make(map[string]interface{})
	request["AclResourceName"] = parts[2]
	request["Username"] = parts[1]
	request["InstanceId"] = parts[0]
	request["AclResourceType"] = parts[3]
	request["AclResourcePatternType"] = parts[5]
	request["AclOperationType"] = parts[4]
	request["RegionId"] = client.RegionId
