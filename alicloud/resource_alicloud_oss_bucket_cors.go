...
func resourceAliCloudOssBucketCorsCreate(d *schema.ResourceData, meta interface{}) error {
	...
	cORSConfiguration := make(map[string]interface{})
	...
	if v := d.Get("cors_rule"); !IsNil(v) {
		if v, ok := d.GetOk("cors_rule"); ok {
			...
			for _, dataLoop := range convertToInterfaceArray(localData) {
				...
				dataLoopMap["ExposeHeader"] = convertToInterfaceArray(dataLoopTmp["expose_headers"])
				dataLoopMap["AllowedMethod"] = convertToInterfaceArray(dataLoopTmp["allowed_methods"])
				dataLoopMap["AllowedHeader"] = convertToInterfaceArray(dataLoopTmp["allowed_headers"])
				dataLoopMap["MaxAgeSeconds"] = dataLoopTmp["max_age_seconds"]
				dataLoopMap["AllowedOrigin"] = convertToInterfaceArray(dataLoopTmp["allowed_origins"])
				localMaps = append(localMaps, dataLoopMap)
			}
			cORSConfiguration["CORSRule"] = localMaps
		}

	}

	if v, ok := d.GetOk("response_vary"); ok {
		cORSConfiguration["ResponseVary"] = v
	}

	request["CORSConfiguration"] = cORSConfiguration
	...
	stateConf := BuildStateConf([]string{}, []string{fmt.Sprint(d.Get("cors_rule"))}, d.Timeout(schema.TimeoutCreate), 5*time.Second, ossServiceV2.OssBucketCorsStateRefreshFunc(d.Id(), "", []string{}))
...
func resourceAliCloudOssBucketCorsRead(d *schema.ResourceData, meta interface{}) error {
	...
	if cORSRuleRaw != nil {
		for _, cORSRuleChildRaw := range convertToInterfaceArray(cORSRuleRaw) {
...
func resourceAliCloudOssBucketCorsUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if v := d.Get("cors_rule"); !IsNil(v) || d.HasChange("cors_rule") {
		if v, ok := d.GetOk("cors_rule"); ok {
			localData, err := jsonpath.Get("$", v)
			if err != nil {
				localData = make([]interface{}, 0)
	...
	if update {
		...
		ossServiceV2 := OssServiceV2{client}
		stateConf := BuildStateConf([]string{}, []string{fmt.Sprint(d.Get("cors_rule"))}, d.Timeout(schema.TimeoutUpdate), 5*time.Second, ossServiceV2.OssBucketCorsStateRefreshFunc(d.Id(), "", []string{}))
		if _, err := stateConf.WaitForState(); err != nil {
			return WrapErrorf(err, IdMsg, d.Id())
		}
	}
