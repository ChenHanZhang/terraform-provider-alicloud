...
func resourceAliCloudDrdsPolardbxInstanceDelete(d *schema.ResourceData, meta interface{}) error {
	...
	drdsServiceV2 := DrdsServiceV2{client}
	stateConf := BuildStateConf([]string{}, []string{""}, d.Timeout(schema.TimeoutDelete), 30*time.Minute, drdsServiceV2.DescribeAsyncDrdsPolardbxInstanceStateRefreshFunc(d, response, "$.Items", []string{}))
