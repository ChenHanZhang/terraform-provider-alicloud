...
// Case TransitRouterPolicyTableAssociation生命周期_测试中_副本1774925809507 12707
func TestAccAliCloudCenTransitRouterPolicyTableAssociation_basic12707(t *testing.T) {
	var v map[string]interface{}
	resourceId := "alicloud_cen_transit_router_policy_table_association.default"
	ra := resourceAttrInit(resourceId, AlicloudCenTransitRouterPolicyTableAssociationMap12707)
	rc := resourceCheckInitWithDescribeMethod(resourceId, &v, func() interface{} {
		return &CenServiceV2{testAccProvider.Meta().(*connectivity.AliyunClient)}
	}, "DescribeCenTransitRouterPolicyTableAssociation")
	rac := resourceAttrCheckInit(rc, ra)
	...
	rand := acctest.RandIntRange(10000, 99999)
	name := fmt.Sprintf("tfacccen%d", rand)
	testAccConfig := resourceTestAccConfigFunc(resourceId, name, AlicloudCenTransitRouterPolicyTableAssociationBasicDependence12707)
	resource.Test(t, resource.TestCase{
		PreCheck: func() {
			testAccPreCheck(t)
		},
...
var AlicloudCenTransitRouterPolicyTableAssociationMap12707 = map[string]string{
	"status": CHECKSET,
}

func AlicloudCenTransitRouterPolicyTableAssociationBasicDependence12707(name string) string {
	return fmt.Sprintf(`
variable "name" {
    default = "%s"
}
...
resource "alicloud_cen_transit_router_vpc_attachment" "defaultb3bDZ6" {
  zone_mappings {
    vswitch_id = "vsw-h5wpq0fwvxq80eln5fa1d"
    zone_id    = "cn-wulanchabu-test-6a"
  }
  vpc_id            = "vpc-h5wer6aocmwj6fy253pn8"
  transit_router_id = alicloud_cen_transit_router.defaultlMj6lY.transit_router_id
}
...
// Case TransitRouterPolicyTableAssociation生命周期_测试中 12704
func TestAccAliCloudCenTransitRouterPolicyTableAssociation_basic12704(t *testing.T) {
	var v map[string]interface{}
	resourceId := "alicloud_cen_transit_router_policy_table_association.default"
	ra := resourceAttrInit(resourceId, AlicloudCenTransitRouterPolicyTableAssociationMap12704)
	rc := resourceCheckInitWithDescribeMethod(resourceId, &v, func() interface{} {
		return &CenServiceV2{testAccProvider.Meta().(*connectivity.AliyunClient)}
	}, "DescribeCenTransitRouterPolicyTableAssociation")
	rac := resourceAttrCheckInit(rc, ra)
	...
	rand := acctest.RandIntRange(10000, 99999)
	name := fmt.Sprintf("tfacccen%d", rand)
	testAccConfig := resourceTestAccConfigFunc(resourceId, name, AlicloudCenTransitRouterPolicyTableAssociationBasicDependence12704)
	resource.Test(t, resource.TestCase{
		PreCheck: func() {
			testAccPreCheck(t)
		},
...
var AlicloudCenTransitRouterPolicyTableAssociationMap12704 = map[string]string{
	"status": CHECKSET,
}

func AlicloudCenTransitRouterPolicyTableAssociationBasicDependence12704(name string) string {
	return fmt.Sprintf(`
variable "name" {
    default = "%s"
}
