...
for _, dataLoop := range convertToInterfaceArray(localData) {
    ...
    if err != nil {
        ...
        stateConf := BuildStateConf([]string{}, []string{fmt.Sprint(d.Get("id"))}, d.Timeout(schema.TimeoutCreate),  5 * time.Second ,ossServiceV2.OssBucketOverwriteConfigStateRefreshFunc(d.Id(), "$.Rule[0].ID", []string {}))
...
for _, dataLoop := range convertToInterfaceArray(localData) {
    ...
    dataLoopMap := make(map[string]interface{})
    	                                        if !IsNil(dataLoopTmp["principals"]) { localData1 := make(map[string]interface{})
                                principal1, _ := jsonpath.Get("$[0].principal", dataLoopTmp["principals"])
        if principal1 != nil&&  principal1 != ""  {
                            localData1["Principal"] = principal1                }
                        if len(localData1) > 0 {
    ...
    overwriteConfiguration["Rule"] = localMaps
        ...
        if err != nil {
            return WrapErrorf(err, DefaultErrorMsg, d.Id(), action, AlibabaCloudSdkGoERROR)
                    ...
                    ossServiceV2 := OssServiceV2{client}
            stateConf := BuildStateConf([]string{}, []string{fmt.Sprint(d.Get("id"))}, d.Timeout(schema.TimeoutUpdate),  5 * time.Second ,ossServiceV2.OssBucketOverwriteConfigStateRefreshFunc(d.Id(), "$.Rule[0].ID", []string {}))
            if _, err := stateConf.WaitForState(); err != nil {
                return WrapErrorf(err, IdMsg, d.Id())
            }
                                }
