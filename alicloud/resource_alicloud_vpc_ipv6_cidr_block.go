...
func resourceAliCloudVpcIpv6CidrBlockRead(d *schema.ResourceData, meta interface{}) error {
	...
	vpcChildRawObj, _ := jsonpath.Get("$$.Vpcs.Vpc[*]", objectRaw)
	vpcChildRaw := make([]interface{}, 0)
	if vpcChildRawObj != nil {
		vpcChildRaw = convertToInterfaceArray(vpcChildRawObj)
	}

	d.Set("region_id", vpcChildRaw["RegionId"])
	d.Set("vpc_id", vpcChildRaw["VpcId"])
