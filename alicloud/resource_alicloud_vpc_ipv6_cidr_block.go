...
func resourceAliCloudVpcIpv6CidrBlockRead(d *schema.ResourceData, meta interface{}) error {
	...
	ipv6CidrBlockRawArrayObj, _ := jsonpath.Get("$.Ipv6CidrBlocks.Ipv6CidrBlock[*]", objectRaw)
	ipv6CidrBlockRawArray := make([]interface{}, 0)
	if ipv6CidrBlockRawArrayObj != nil {
		ipv6CidrBlockRawArray = convertToInterfaceArray(ipv6CidrBlockRawArrayObj)
	}
	ipv6CidrBlockRaw := make(map[string]interface{})
	if len(ipv6CidrBlockRawArray) > 0 {
		ipv6CidrBlockRaw = ipv6CidrBlockRawArray[0].(map[string]interface{})
	}
