	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"project": {
				...
				Optional:   true,
				Computed:   true,
				Deprecated: "Field 'project' has been deprecated since provider version 1.274.0. New field 'project_name' instead.",
				ForceNew:   true,
			},
			"name": {
				Type:       schema.TypeString,
				Optional:   true,
				Computed:   true,
				Deprecated: "Field 'name' has been deprecated since provider version 1.274.0. New field 'logstore_name' instead.",
				ForceNew:   true,
			},
			"shards": {
				Type:     schema.TypeList,
...
func resourceAliCloudSlsLogStoreCreate(d *schema.ResourceData, meta interface{}) error {
	...
	encrypt_conf := make(map[string]interface{})
	...
	if v := d.Get("encrypt_conf"); !IsNil(v) {
		...
		if len(user_cmk_info) > 0 {
			encrypt_conf["user_cmk_info"] = user_cmk_info
		}
		encryptType, _ := jsonpath.Get("$[0].encrypt_type", v)
		if encryptType != nil && encryptType != "" {
			encrypt_conf["encrypt_type"] = encryptType
		}
		enable1, _ := jsonpath.Get("$[0].enable", v)
		if enable1 != nil && enable1 != "" {
			encrypt_conf["enable"] = enable1
		}

		request["encrypt_conf"] = encrypt_conf
	}
...
func resourceAliCloudSlsLogStoreRead(d *schema.ResourceData, meta interface{}) error {
	...
	objectRaw, err = slsServiceV2.DescribeLogStoreListTagResources(d.Id())
...
func resourceAliCloudSlsLogStoreUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := d.GetOkExists("infrequent_access_ttl"); ok || d.HasChange("infrequent_access_ttl") {
		request["infrequentAccessTTL"] = v
	}
	if !d.IsNewResource() && d.HasChange("hot_ttl") {
		update = true
	}
	if v, ok := d.GetOkExists("hot_ttl"); ok || d.HasChange("hot_ttl") {
		request["hot_ttl"] = v
	}
	if !d.IsNewResource() && d.HasChange("append_meta") {
		update = true
	}
	if v, ok := d.GetOkExists("append_meta"); ok || d.HasChange("append_meta") {
		request["appendMeta"] = v
	}
	...
	if v, ok := d.GetOkExists("max_split_shard_count"); ok || d.HasChange("max_split_shard_count") {
		request["maxSplitShard"] = v
	}
	if !d.IsNewResource() && d.HasChange("enable_web_tracking") {
		update = true
	}
	if v, ok := d.GetOkExists("enable_web_tracking"); ok || d.HasChange("enable_web_tracking") {
		request["enable_tracking"] = v
	}
	...
	if v, ok := d.GetOk("encrypt_conf"); ok || d.HasChange("encrypt_conf") {
		encryptConfEncryptTypeJsonPath, err := jsonpath.Get("$[0].encrypt_type", v)
		if err == nil && encryptConfEncryptTypeJsonPath != "" {
			request["encryptType"] = encryptConfEncryptTypeJsonPath
		}
	}
	if !d.IsNewResource() && d.HasChange("encrypt_conf.0.enable") {
		update = true
	}
	encryptConfEnableJsonPath, err := jsonpath.Get("$[0].enable", d.Get("encrypt_conf"))
	if err == nil {
		request["enable"] = encryptConfEnableJsonPath
	}
	...
	parts = strings.Split(d.Id(), ":")
	logstore = parts[1]
	action = fmt.Sprintf("/logstores/%s/config", logstore)
	request = make(map[string]interface{})
	query = make(map[string]*string)
	body = make(map[string]interface{})
	hostMap = make(map[string]*string)
	...
	if update {
		wait := incrementalWait(3*time.Second, 0*time.Second)
		err = resource.Retry(d.Timeout(schema.TimeoutUpdate), func() *resource.RetryError {
			response, err = client.Do("Sls", roaParam("PUT", "2020-12-30", "PutLogStoreConfig", action), query, body, nil, hostMap, false)
			if err != nil {
				if NeedRetry(err) {
					wait()
					return resource.RetryableError(err)
