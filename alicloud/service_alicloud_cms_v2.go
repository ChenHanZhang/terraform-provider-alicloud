...
// DescribeCmsPrometheusCluster <<< Encapsulated get interface for Cms PrometheusCluster.

func (s *CmsServiceV2) DescribeCmsPrometheusCluster(id string) (object map[string]interface{}, err error) {
	client := s.client
	var request map[string]interface{}
	var response map[string]interface{}
	var query map[string]*string
	prometheusClusterName := id
	request = make(map[string]interface{})
	query = make(map[string]*string)
	...
	if err != nil {
		if IsExpectedErrors(err, []string{"404"}) {
			return object, WrapErrorf(NotFoundErr("PrometheusCluster", id), NotFoundMsg, response)
		}
		return object, WrapErrorf(err, DefaultErrorMsg, id, action, AlibabaCloudSdkGoERROR)
	}

	v, err := jsonpath.Get("$.prometheusCluster", response)
	if err != nil {
		return object, WrapErrorf(err, FailedGetAttributeMsg, id, "$.prometheusCluster", response)
	}

	return v.(map[string]interface{}), nil
}

func (s *CmsServiceV2) CmsPrometheusClusterStateRefreshFunc(id string, field string, failStates []string) resource.StateRefreshFunc {
	return s.CmsPrometheusClusterStateRefreshFuncWithApi(id, field, failStates, s.DescribeCmsPrometheusCluster)
}

func (s *CmsServiceV2) CmsPrometheusClusterStateRefreshFuncWithApi(id string, field string, failStates []string, call func(id string) (map[string]interface{}, error)) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		object, err := call(id)
		if err != nil {
			if NotFoundError(err) {
...
// DescribeCmsPrometheusCluster >>> Encapsulated.
