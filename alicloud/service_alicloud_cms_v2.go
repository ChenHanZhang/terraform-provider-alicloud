...
// DescribeCmsDeliveryTask <<< Encapsulated get interface for Cms DeliveryTask.

func (s *CmsServiceV2) DescribeCmsDeliveryTask(id string) (object map[string]interface{}, err error) {
	client := s.client
	var request map[string]interface{}
	var response map[string]interface{}
	var query map[string]*string
	...
	if err != nil {
		if IsExpectedErrors(err, []string{"404"}) {
			return object, WrapErrorf(NotFoundErr("DeliveryTask", id), NotFoundMsg, response)
		}
		return object, WrapErrorf(err, DefaultErrorMsg, id, action, AlibabaCloudSdkGoERROR)
	}

	v, err := jsonpath.Get("$.deliveryTask", response)
	if err != nil {
		return object, WrapErrorf(err, FailedGetAttributeMsg, id, "$.deliveryTask", response)
	}

	return v.(map[string]interface{}), nil
}

func (s *CmsServiceV2) CmsDeliveryTaskStateRefreshFunc(id string, field string, failStates []string) resource.StateRefreshFunc {
	return s.CmsDeliveryTaskStateRefreshFuncWithApi(id, field, failStates, s.DescribeCmsDeliveryTask)
}

func (s *CmsServiceV2) CmsDeliveryTaskStateRefreshFuncWithApi(id string, field string, failStates []string, call func(id string) (map[string]interface{}, error)) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		object, err := call(id)
		if err != nil {
			if NotFoundError(err) {
...
// DescribeCmsDeliveryTask >>> Encapsulated.
