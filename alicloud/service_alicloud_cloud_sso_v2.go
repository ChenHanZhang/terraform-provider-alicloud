...
func (s *CloudSSOServiceV2) DescribeCloudSsoUserAttachment(id string) (object map[string]interface{}, err error) {
	...
	v, err := jsonpath.Get("$.JoinedGroups[*]", response)
	if err != nil {
		return object, WrapErrorf(err, FailedGetAttributeMsg, id, "$.JoinedGroups[*]", response)
	}
	...
	for _, v := range result {
		...
		if fmt.Sprint(item["GroupId"]) != parts[1] {
			continue
		}
		return item, nil
	}
...
func (s *CloudSSOServiceV2) CloudSsoUserAttachmentStateRefreshFunc(id string, field string, failStates []string) resource.StateRefreshFunc {
	return s.CloudSsoUserAttachmentStateRefreshFuncWithApi(id, field, failStates, s.DescribeCloudSsoUserAttachment)
}

func (s *CloudSSOServiceV2) CloudSsoUserAttachmentStateRefreshFuncWithApi(id string, field string, failStates []string, call func(id string) (map[string]interface{}, error)) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		object, err := call(id)
		if err != nil {
			if NotFoundError(err) {
