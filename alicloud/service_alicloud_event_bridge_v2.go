...
// DescribeEventBridgeTable <<< Encapsulated get interface for EventBridge Table.

func (s *EventBridgeServiceV2) DescribeEventBridgeTable(id string) (object map[string]interface{}, err error) {
	client := s.client
	var request map[string]interface{}
	var response map[string]interface{}
	var query map[string]interface{}
	...
	addDebug(action, response, request)
	if err != nil {
		if IsExpectedErrors(err, []string{"TableNotExist"}) {
			return object, WrapErrorf(NotFoundErr("Table", id), NotFoundMsg, response)
		}
		return object, WrapErrorf(err, DefaultErrorMsg, id, action, AlibabaCloudSdkGoERROR)
	}
...
func (s *EventBridgeServiceV2) EventBridgeTableStateRefreshFunc(id string, field string, failStates []string) resource.StateRefreshFunc {
	return s.EventBridgeTableStateRefreshFuncWithApi(id, field, failStates, s.DescribeEventBridgeTable)
}

func (s *EventBridgeServiceV2) EventBridgeTableStateRefreshFuncWithApi(id string, field string, failStates []string, call func(id string) (map[string]interface{}, error)) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		object, err := call(id)
		if err != nil {
			if NotFoundError(err) {
...
// DescribeEventBridgeTable >>> Encapsulated.
