...
// DescribeCenRouteMap <<< Encapsulated get interface for Cen RouteMap.

func (s *CenServiceV2) DescribeCenRouteMap(id string) (object map[string]interface{}, err error) {
	client := s.client
	var request map[string]interface{}
	var response map[string]interface{}
	var query map[string]interface{}
	request = make(map[string]interface{})
	query = make(map[string]interface{})
	request["RouteMapIds.1"] = id

	action := "ListRouteMaps"
	request["ClientToken"] = buildClientToken(action)
	...
	v, err := jsonpath.Get("$.RouteMaps[*]", response)
	if err != nil {
		return object, WrapErrorf(err, FailedGetAttributeMsg, id, "$.RouteMaps[*]", response)
	}

	if len(v.([]interface{})) == 0 {
		return object, WrapErrorf(NotFoundErr("RouteMap", id), NotFoundMsg, response)
	}

	return v.([]interface{})[0].(map[string]interface{}), nil
}

func (s *CenServiceV2) CenRouteMapStateRefreshFunc(id string, field string, failStates []string) resource.StateRefreshFunc {
	return s.CenRouteMapStateRefreshFuncWithApi(id, field, failStates, s.DescribeCenRouteMap)
}

func (s *CenServiceV2) CenRouteMapStateRefreshFuncWithApi(id string, field string, failStates []string, call func(id string) (map[string]interface{}, error)) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		object, err := call(id)
		if err != nil {
			if NotFoundError(err) {
...
// DescribeCenRouteMap >>> Encapsulated.
