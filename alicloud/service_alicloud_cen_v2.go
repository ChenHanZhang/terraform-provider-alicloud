...
// DescribeCenTransitRouterPolicyTable <<< Encapsulated get interface for Cen TransitRouterPolicyTable.

func (s *CenServiceV2) DescribeCenTransitRouterPolicyTable(id string) (object map[string]interface{}, err error) {
	client := s.client
	var request map[string]interface{}
	var response map[string]interface{}
	var query map[string]interface{}
	...
	action := "DescribeTransitRouterPolicyTables"
	...
	v, err := jsonpath.Get("$.PolicyTables[*]", response)
	if err != nil {
		return object, WrapErrorf(err, FailedGetAttributeMsg, id, "$.PolicyTables[*]", response)
	}

	if len(v.([]interface{})) == 0 {
		return object, WrapErrorf(NotFoundErr("TransitRouterPolicyTable", id), NotFoundMsg, response)
	}

	return v.([]interface{})[0].(map[string]interface{}), nil
}

func (s *CenServiceV2) CenTransitRouterPolicyTableStateRefreshFunc(id string, field string, failStates []string) resource.StateRefreshFunc {
	return s.CenTransitRouterPolicyTableStateRefreshFuncWithApi(id, field, failStates, s.DescribeCenTransitRouterPolicyTable)
}

func (s *CenServiceV2) CenTransitRouterPolicyTableStateRefreshFuncWithApi(id string, field string, failStates []string, call func(id string) (map[string]interface{}, error)) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		object, err := call(id)
		if err != nil {
			if NotFoundError(err) {
...
// DescribeCenTransitRouterPolicyTable >>> Encapsulated.
