...
// DescribeCenTransitRouterPolicyBasedRoute <<< Encapsulated get interface for Cen TransitRouterPolicyBasedRoute.

func (s *CenServiceV2) DescribeCenTransitRouterPolicyBasedRoute(id string) (object map[string]interface{}, err error) {
	client := s.client
	var request map[string]interface{}
	var response map[string]interface{}
	var query map[string]interface{}
	...
	action := "DescribeTransitRouterPolicyBasedRoutes"
	...
	v, err := jsonpath.Get("$.PolicyBasedRoutes[*]", response)
	if err != nil {
		return object, WrapErrorf(err, FailedGetAttributeMsg, id, "$.PolicyBasedRoutes[*]", response)
	}

	if len(v.([]interface{})) == 0 {
		return object, WrapErrorf(NotFoundErr("TransitRouterPolicyBasedRoute", id), NotFoundMsg, response)
	}

	return v.([]interface{})[0].(map[string]interface{}), nil
}

func (s *CenServiceV2) CenTransitRouterPolicyBasedRouteStateRefreshFunc(id string, field string, failStates []string) resource.StateRefreshFunc {
	return s.CenTransitRouterPolicyBasedRouteStateRefreshFuncWithApi(id, field, failStates, s.DescribeCenTransitRouterPolicyBasedRoute)
}

func (s *CenServiceV2) CenTransitRouterPolicyBasedRouteStateRefreshFuncWithApi(id string, field string, failStates []string, call func(id string) (map[string]interface{}, error)) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		object, err := call(id)
		if err != nil {
			if NotFoundError(err) {
...
// DescribeCenTransitRouterPolicyBasedRoute >>> Encapsulated.
