...
// Case 接入RMC_换账号 3519
func TestAccAliCloudVpcNetworkAcl_basic3519(t *testing.T) {
	var v map[string]interface{}
	resourceId := "alicloud_network_acl.default"
	ra := resourceAttrInit(resourceId, AlicloudVpcNetworkAclMap3519)
	rc := resourceCheckInitWithDescribeMethod(resourceId, &v, func() interface{} {
	...
	testAccConfig := resourceTestAccConfigFunc(resourceId, name, AlicloudVpcNetworkAclBasicDependence3519)
	resource.Test(t, resource.TestCase{
		PreCheck: func() {
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheck(map[string]string{
						...
						"vpc_id":                CHECKSET,
						"network_acl_name":      name,
						"ingress_acl_entries.#": "0",
						"egress_acl_entries.#":  "0",
						"resources.#":           "0",
					}),
				),
			},
...
var AlicloudVpcNetworkAclMap3519 = map[string]string{
	"status":      CHECKSET,
	"create_time": CHECKSET,
	"region_id":   CHECKSET,
}

func AlicloudVpcNetworkAclBasicDependence3519(name string) string {
	return fmt.Sprintf(`
variable "name" {
    default = "%s"
}
