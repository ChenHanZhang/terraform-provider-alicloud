...
func resourceAliCloudVpcVSwitchCidrReservationCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return WrapErrorf(err, DefaultErrorMsg, "alicloud_vpc_v_switch_cidr_reservation", action, AlibabaCloudSdkGoERROR)
	}

	d.SetId(fmt.Sprintf("%v:%v", request["VSwitchId"], response["VSwitchCidrReservationId"]))

	vpcServiceV2 := VpcServiceV2{client}
	stateConf := BuildStateConf([]string{}, []string{"Assigned"}, d.Timeout(schema.TimeoutCreate), 5*time.Second, vpcServiceV2.VpcVSwitchCidrReservationStateRefreshFunc(d.Id(), "Status", []string{}))
...
func resourceAliCloudVpcVSwitchCidrReservationRead(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		if !d.IsNewResource() && NotFoundError(err) {
			log.Printf("[DEBUG] Resource alicloud_vpc_v_switch_cidr_reservation DescribeVpcVSwitchCidrReservation Failed!!! %s", err)
			d.SetId("")
			return nil
		}
		return WrapError(err)
...
func resourceAliCloudVpcVSwitchCidrReservationUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChange("cidr_reservation_description") {
		update = true
		request["VSwitchCidrReservationDescription"] = d.Get("cidr_reservation_description")
	}

	if d.HasChange("vswitch_cidr_reservation_name") {
		update = true
		request["VSwitchCidrReservationName"] = d.Get("vswitch_cidr_reservation_name")
	}
...
func resourceAliCloudVpcVSwitchCidrReservationDelete(d *schema.ResourceData, meta interface{}) error {
	...
	vpcServiceV2 := VpcServiceV2{client}
	stateConf := BuildStateConf([]string{}, []string{}, d.Timeout(schema.TimeoutDelete), 5*time.Second, vpcServiceV2.VpcVSwitchCidrReservationStateRefreshFunc(d.Id(), "Status", []string{}))
	if _, err := stateConf.WaitForState(); err != nil {
		return WrapErrorf(err, IdMsg, d.Id())
	}
	return nil
