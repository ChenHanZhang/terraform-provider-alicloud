...
// DescribeActionTrailAnalysisTask <<< Encapsulated get interface for ActionTrail AnalysisTask.

func (s *ActionTrailServiceV2) DescribeActionTrailAnalysisTask(id string) (object map[string]interface{}, err error) {
	client := s.client
	var request map[string]interface{}
	var response map[string]interface{}
	var query map[string]interface{}
	request = make(map[string]interface{})
	query = make(map[string]interface{})
	request["AnalysisTaskId"] = id

	action := "DescribeAnalysisTask"
	...
	addDebug(action, response, request)
	if err != nil {
		if IsExpectedErrors(err, []string{"TaskNotFound"}) {
			return object, WrapErrorf(NotFoundErr("AnalysisTask", id), NotFoundMsg, response)
		}
		return object, WrapErrorf(err, DefaultErrorMsg, id, action, AlibabaCloudSdkGoERROR)
	}
...
func (s *ActionTrailServiceV2) ActionTrailAnalysisTaskStateRefreshFunc(id string, field string, failStates []string) resource.StateRefreshFunc {
	return s.ActionTrailAnalysisTaskStateRefreshFuncWithApi(id, field, failStates, s.DescribeActionTrailAnalysisTask)
}

func (s *ActionTrailServiceV2) ActionTrailAnalysisTaskStateRefreshFuncWithApi(id string, field string, failStates []string, call func(id string) (map[string]interface{}, error)) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		object, err := call(id)
		if err != nil {
			if NotFoundError(err) {
...
// DescribeActionTrailAnalysisTask >>> Encapsulated.
