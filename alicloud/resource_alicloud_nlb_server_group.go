	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"connection_drain": {
				...
				Optional:   true,
				Computed:   true,
				Deprecated: "Field 'connection_drain' has been deprecated since provider version 1.212.0. New field 'connection_drain_enabled' instead.",
			},
		},
	}
}
...
func resourceAliCloudNlbServerGroupCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := d.GetOk("health_check"); ok {
		jsonPathResult9, err := jsonpath.Get("$[0].health_check_enabled", v)
		if err == nil && jsonPathResult9 != "" {
			request["HealthCheckConfig.HealthCheckEnabled"] = jsonPathResult9
		}
	}
	if v, ok := d.GetOk("health_check"); ok {
		jsonPathResult10, err := jsonpath.Get("$[0].health_check_type", v)
		if err == nil && jsonPathResult10 != "" {
			request["HealthCheckConfig.HealthCheckType"] = jsonPathResult10
		}
	}
	if v, ok := d.GetOk("health_check"); ok {
		jsonPathResult11, err := jsonpath.Get("$[0].health_check_connect_port", v)
		if err == nil && jsonPathResult11 != "" {
			request["HealthCheckConfig.HealthCheckConnectPort"] = jsonPathResult11
		}
	}
	if v, ok := d.GetOk("health_check"); ok {
		jsonPathResult12, err := jsonpath.Get("$[0].healthy_threshold", v)
		if err == nil && jsonPathResult12 != "" {
			request["HealthCheckConfig.HealthyThreshold"] = jsonPathResult12
		}
	}
	if v, ok := d.GetOk("health_check"); ok {
		jsonPathResult13, err := jsonpath.Get("$[0].unhealthy_threshold", v)
		if err == nil && jsonPathResult13 != "" {
			request["HealthCheckConfig.UnhealthyThreshold"] = jsonPathResult13
		}
	}
	if v, ok := d.GetOk("health_check"); ok {
		jsonPathResult14, err := jsonpath.Get("$[0].health_check_connect_timeout", v)
		if err == nil && jsonPathResult14 != "" {
			request["HealthCheckConfig.HealthCheckConnectTimeout"] = jsonPathResult14
		}
	}
	if v, ok := d.GetOk("health_check"); ok {
		jsonPathResult15, err := jsonpath.Get("$[0].health_check_interval", v)
		if err == nil && jsonPathResult15 != "" {
			request["HealthCheckConfig.HealthCheckInterval"] = jsonPathResult15
		}
	}
	if v, ok := d.GetOk("health_check"); ok {
	...
	if v, ok := d.GetOk("health_check"); ok {
		jsonPathResult17, err := jsonpath.Get("$[0].health_check_url", v)
		if err == nil && jsonPathResult17 != "" {
			request["HealthCheckConfig.HealthCheckUrl"] = jsonPathResult17
		}
	}
	if v, ok := d.GetOk("health_check"); ok {
		jsonPathResult18, err := jsonpath.Get("$[0].http_check_method", v)
		if err == nil && jsonPathResult18 != "" {
			request["HealthCheckConfig.HttpCheckMethod"] = jsonPathResult18
		}
	}
...
func resourceAliCloudNlbServerGroupUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if !d.IsNewResource() && d.HasChange("health_check") {
		update = true
		jsonPathResult4, err := jsonpath.Get("$[0].health_check_enabled", d.Get("health_check"))
		if err == nil {
	...
	if !d.IsNewResource() && d.HasChange("health_check") {
		update = true
		jsonPathResult5, err := jsonpath.Get("$[0].health_check_type", d.Get("health_check"))
		if err == nil {
			request["HealthCheckConfig.HealthCheckType"] = jsonPathResult5
		}
	}

	if !d.IsNewResource() && d.HasChange("health_check") {
		update = true
		jsonPathResult6, err := jsonpath.Get("$[0].health_check_connect_port", d.Get("health_check"))
		if err == nil {
			request["HealthCheckConfig.HealthCheckConnectPort"] = jsonPathResult6
		}
	}

	if !d.IsNewResource() && d.HasChange("health_check") {
		update = true
		jsonPathResult7, err := jsonpath.Get("$[0].healthy_threshold", d.Get("health_check"))
		if err == nil {
			request["HealthCheckConfig.HealthyThreshold"] = jsonPathResult7
		}
	}

	if !d.IsNewResource() && d.HasChange("health_check") {
		update = true
		jsonPathResult8, err := jsonpath.Get("$[0].unhealthy_threshold", d.Get("health_check"))
		if err == nil {
	...
	if !d.IsNewResource() && d.HasChange("health_check") {
		update = true
		jsonPathResult9, err := jsonpath.Get("$[0].health_check_connect_timeout", d.Get("health_check"))
		if err == nil {
			request["HealthCheckConfig.HealthCheckConnectTimeout"] = jsonPathResult9
		}
	}

	if !d.IsNewResource() && d.HasChange("health_check") {
		update = true
		jsonPathResult10, err := jsonpath.Get("$[0].health_check_interval", d.Get("health_check"))
		if err == nil {
			request["HealthCheckConfig.HealthCheckInterval"] = jsonPathResult10
		}
	}

	if !d.IsNewResource() && d.HasChange("health_check") {
		update = true
		jsonPathResult11, err := jsonpath.Get("$[0].health_check_domain", d.Get("health_check"))
		if err == nil {
			request["HealthCheckConfig.HealthCheckDomain"] = jsonPathResult11
		}
	}

	if !d.IsNewResource() && d.HasChange("health_check") {
		update = true
		jsonPathResult12, err := jsonpath.Get("$[0].health_check_url", d.Get("health_check"))
		if err == nil {
			request["HealthCheckConfig.HealthCheckUrl"] = jsonPathResult12
		}
	}

	if !d.IsNewResource() && d.HasChange("health_check") {
		update = true
		jsonPathResult13, err := jsonpath.Get("$[0].http_check_method", d.Get("health_check"))
		if err == nil {
			request["HealthCheckConfig.HttpCheckMethod"] = jsonPathResult13
		}
	}
