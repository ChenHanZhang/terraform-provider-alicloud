...
func (s *AlikafkaServiceV2) AlikafkaSaslAclStateRefreshFuncWithApi(id string, field string, failStates []string, call func(id string) (map[string]interface{}, error)) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		...
		object["AclOperationType"] = convertAlikafkaSaslAclKafkaAclListKafkaAclVOAclOperationTypeResponse(object["AclOperationType"])
		object["AclResourceType"] = convertAlikafkaSaslAclKafkaAclListKafkaAclVOAclResourceTypeResponse(object["AclResourceType"])
		v, err := jsonpath.Get(field, object)
		currentStatus := fmt.Sprint(v)
