...
// DescribeAdbLakeStorage <<< Encapsulated get interface for Adb LakeStorage.

func (s *AdbServiceV2) DescribeAdbLakeStorage(id string) (object map[string]interface{}, err error) {
	client := s.client
	var request map[string]interface{}
	var response map[string]interface{}
	var query map[string]interface{}
	...
	if InArray(fmt.Sprint(code), []string{"InvalidInput"}) {
		return object, WrapErrorf(NotFoundErr("LakeStorage", id), NotFoundMsg, response)
	}
...
func (s *AdbServiceV2) AdbLakeStorageStateRefreshFunc(id string, field string, failStates []string) resource.StateRefreshFunc {
	return s.AdbLakeStorageStateRefreshFuncWithApi(id, field, failStates, s.DescribeAdbLakeStorage)
}

func (s *AdbServiceV2) AdbLakeStorageStateRefreshFuncWithApi(id string, field string, failStates []string, call func(id string) (map[string]interface{}, error)) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		object, err := call(id)
		if err != nil {
			if NotFoundError(err) {
...
// DescribeAdbLakeStorage >>> Encapsulated.
