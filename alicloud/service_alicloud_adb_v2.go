...
// DescribeAdbPipelineJob <<< Encapsulated get interface for Adb PipelineJob.

func (s *AdbServiceV2) DescribeAdbPipelineJob(id string) (object map[string]interface{}, err error) {
	client := s.client
	var request map[string]interface{}
	var response map[string]interface{}
	var query map[string]interface{}
	...
	if InArray(fmt.Sprint(code), []string{"InvalidInput"}) {
		return object, WrapErrorf(NotFoundErr("PipelineJob", id), NotFoundMsg, response)
	}
...
func (s *AdbServiceV2) AdbPipelineJobStateRefreshFunc(id string, field string, failStates []string) resource.StateRefreshFunc {
	return s.AdbPipelineJobStateRefreshFuncWithApi(id, field, failStates, s.DescribeAdbPipelineJob)
}

func (s *AdbServiceV2) AdbPipelineJobStateRefreshFuncWithApi(id string, field string, failStates []string, call func(id string) (map[string]interface{}, error)) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		object, err := call(id)
		if err != nil {
			if NotFoundError(err) {
...
// DescribeAdbPipelineJob >>> Encapsulated.
