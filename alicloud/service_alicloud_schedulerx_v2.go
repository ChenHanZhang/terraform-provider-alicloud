...
// DescribeSchedulerxWorkFlow <<< Encapsulated get interface for Schedulerx WorkFlow.

func (s *SchedulerxServiceV2) DescribeSchedulerxWorkFlow(id string) (object map[string]interface{}, err error) {
	client := s.client
	var request map[string]interface{}
	var response map[string]interface{}
	var query map[string]interface{}
	...
	v, err := jsonpath.Get("$.Data.WorkFlowInfo", response)
	if err != nil {
		return object, WrapErrorf(err, FailedGetAttributeMsg, id, "$.Data.WorkFlowInfo", response)
	}

	currentStatus := v.(map[string]interface{})["Status"]
	if fmt.Sprint(currentStatus) == "removed" {
		return object, WrapErrorf(NotFoundErr("WorkFlow", id), NotFoundMsg, response)
	}

	return v.(map[string]interface{}), nil
}

func (s *SchedulerxServiceV2) SchedulerxWorkFlowStateRefreshFunc(id string, field string, failStates []string) resource.StateRefreshFunc {
	return s.SchedulerxWorkFlowStateRefreshFuncWithApi(id, field, failStates, s.DescribeSchedulerxWorkFlow)
}

func (s *SchedulerxServiceV2) SchedulerxWorkFlowStateRefreshFuncWithApi(id string, field string, failStates []string, call func(id string) (map[string]interface{}, error)) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		object, err := call(id)
		if err != nil {
			if NotFoundError(err) {
		...
		object["Status"] = convertSchedulerxWorkFlowDataWorkFlowInfoStatusResponse(object["Status"])
		object["TimeType"] = convertSchedulerxWorkFlowDataWorkFlowInfoTimeTypeResponse(object["TimeType"])
		v, err := jsonpath.Get(field, object)
		currentStatus := fmt.Sprint(v)
...
// DescribeSchedulerxWorkFlow >>> Encapsulated.
