...
// Case resourceCase_20260408_disk01_clone_0 12739
func TestAccAliCloudRdsCustomDisk_basic12739(t *testing.T) {
	var v map[string]interface{}
	resourceId := "alicloud_rds_custom_disk.default"
	ra := resourceAttrInit(resourceId, AlicloudRdsCustomDiskMap12739)
	...
	testAccConfig := resourceTestAccConfigFunc(resourceId, name, AlicloudRdsCustomDiskBasicDependence12739)
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				ImportStateVerifyIgnore: []string{"auto_pay", "auto_renew", "disk_category", "dry_run", "instance_charge_type", "instance_id", "period", "period_unit", "snapshot_id", "type"},
			},
		},
	})
}

var AlicloudRdsCustomDiskMap12739 = map[string]string{
	"status":      CHECKSET,
	"create_time": CHECKSET,
	"region_id":   CHECKSET,
}

func AlicloudRdsCustomDiskBasicDependence12739(name string) string {
	return fmt.Sprintf(`
variable "name" {
    default = "%s"
}
...
// Case resourceCase_20260408_disk01 12737
func TestAccAliCloudRdsCustomDisk_basic12737(t *testing.T) {
	var v map[string]interface{}
	resourceId := "alicloud_rds_custom_disk.default"
	ra := resourceAttrInit(resourceId, AlicloudRdsCustomDiskMap12737)
	rc := resourceCheckInitWithDescribeMethod(resourceId, &v, func() interface{} {
		return &RdsServiceV2{testAccProvider.Meta().(*connectivity.AliyunClient)}
	}, "DescribeRdsCustomDisk")
	rac := resourceAttrCheckInit(rc, ra)
	...
	rand := acctest.RandIntRange(10000, 99999)
	name := fmt.Sprintf("tfaccrds%d", rand)
	testAccConfig := resourceTestAccConfigFunc(resourceId, name, AlicloudRdsCustomDiskBasicDependence12737)
	resource.Test(t, resource.TestCase{
		PreCheck: func() {
			testAccPreCheck(t)
		},
...
var AlicloudRdsCustomDiskMap12737 = map[string]string{
	"status":      CHECKSET,
	"create_time": CHECKSET,
	"region_id":   CHECKSET,
}

func AlicloudRdsCustomDiskBasicDependence12737(name string) string {
	return fmt.Sprintf(`
variable "name" {
    default = "%s"
}
