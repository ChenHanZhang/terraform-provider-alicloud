...
// DescribePaiWorkspaceConnection <<< Encapsulated get interface for PaiWorkspace Connection.

func (s *PaiWorkspaceServiceV2) DescribePaiWorkspaceConnection(id string) (object map[string]interface{}, err error) {
	client := s.client
	var request map[string]interface{}
	var response map[string]interface{}
	var query map[string]*string
	...
	wait := incrementalWait(3*time.Second, 5*time.Second)
	err = resource.Retry(1*time.Minute, func() *resource.RetryError {
		response, err = client.RoaGet("AIWorkSpace", "2021-02-04", action, query, header, nil)
	...
	addDebug(action, response, request)
	if err != nil {
		if IsExpectedErrors(err, []string{"RequestFailedErrorProblem"}) {
			return object, WrapErrorf(NotFoundErr("Connection", id), NotFoundMsg, response)
		}
		return object, WrapErrorf(err, DefaultErrorMsg, id, action, AlibabaCloudSdkGoERROR)
	}
...
func (s *PaiWorkspaceServiceV2) PaiWorkspaceConnectionStateRefreshFunc(id string, field string, failStates []string) resource.StateRefreshFunc {
	return s.PaiWorkspaceConnectionStateRefreshFuncWithApi(id, field, failStates, s.DescribePaiWorkspaceConnection)
}

func (s *PaiWorkspaceServiceV2) PaiWorkspaceConnectionStateRefreshFuncWithApi(id string, field string, failStates []string, call func(id string) (map[string]interface{}, error)) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		object, err := call(id)
		if err != nil {
			if NotFoundError(err) {
...
// DescribePaiWorkspaceConnection >>> Encapsulated.
