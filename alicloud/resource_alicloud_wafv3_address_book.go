...
func resourceAliCloudWafv3AddressBookRead(d *schema.ResourceData, meta interface{}) error {
	...
	objectRaw, err = wafv3ServiceV2.DescribeAddressBookDescribeAddresses(d.Id())
	if err != nil && !NotFoundError(err) {
		return WrapError(err)
	}

	addressListRawObj, _ := jsonpath.Get("$.AddressList[*]", objectRaw)
	addressListRaw := make([]interface{}, 0)
	if addressListRawObj != nil {
		addressListRaw = convertToInterfaceArray(addressListRawObj)
	}

	d.Set("address_book_id", addressListRaw["RuleId"])

	addressListChildRaw := make([]interface{}, 0)
	if addressListRaw["AddressListChild"] != nil {
		addressListChildRaw = convertToInterfaceArray(addressListRaw["AddressListChild"])
	}

	d.Set("address_list", addressListChildRaw)
...
func resourceAliCloudWafv3AddressBookUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	wafv3ServiceV2 := Wafv3ServiceV2{client}
	objectRaw, _ := wafv3ServiceV2.DescribeWafv3AddressBook(d.Id())

	if d.HasChange("address_list") {
		var err error
		target := d.Get("address_list").(string)
		if target == "[]" {
			parts := strings.Split(d.Id(), ":")
			action := "ClearAddress"
			request = make(map[string]interface{})
	...
	if d.HasChange("address_book_id") {
		update = true
	}
	if v, ok := d.GetOk("address_book_id"); ok {
		rulesDataList["id"] = v
	}

	if d.HasChange("address_book_type") {
		update = true
	}
	if v, ok := d.GetOk("address_book_type"); ok {
		rulesDataList["valueType"] = v
	...
	if d.HasChange("address_book_name") {
		update = true
	}
	if v, ok := d.GetOk("address_book_name"); ok {
		rulesDataList["name"] = v
	...
	jsonString, _ = sjson.Set(jsonString, "Rules.0.id", parts[1])
