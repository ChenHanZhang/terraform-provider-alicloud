...
func resourceAliCloudAlidnsCloudGtmMonitorTemplateCreate(d *schema.ResourceData, meta interface{}) error {
	...
	ispCityNodesDataList := make(map[string]interface{})

	if v, ok := d.GetOk("isp_city_nodes"); ok {
		cityCode1, _ := jsonpath.Get("$.city_code", v)
		if cityCode1 != nil && cityCode1 != "" {
			ispCityNodesDataList["CityCode"] = cityCode1
		}
	}

	if v, ok := d.GetOk("isp_city_nodes"); ok {
		ispCode1, _ := jsonpath.Get("$.isp_code", v)
		if ispCode1 != nil && ispCode1 != "" {
			ispCityNodesDataList["IspCode"] = ispCode1
		}
	}

	IspCityNodesMap := make([]interface{}, 0)
	IspCityNodesMap = append(IspCityNodesMap, ispCityNodesDataList)
	ispCityNodesDataListJson, err := json.Marshal(IspCityNodesMap)
	if err != nil {
		return WrapError(err)
	}
	request["IspCityNodes"] = string(ispCityNodesDataListJson)
