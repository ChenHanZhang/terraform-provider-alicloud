...
// DescribeNATGatewayFullNat <<< Encapsulated get interface for NATGateway FullNat.

func (s *NATGatewayServiceV2) DescribeNATGatewayFullNat(id string) (object map[string]interface{}, err error) {
	client := s.client
	var request map[string]interface{}
	var response map[string]interface{}
	var query map[string]interface{}
	...
	v, err := jsonpath.Get("$.FullNatEntries[*]", response)
	if err != nil {
		return object, WrapErrorf(err, FailedGetAttributeMsg, id, "$.FullNatEntries[*]", response)
	}

	if len(v.([]interface{})) == 0 {
		return object, WrapErrorf(NotFoundErr("FullNat", id), NotFoundMsg, response)
	}

	return v.([]interface{})[0].(map[string]interface{}), nil
}

func (s *NATGatewayServiceV2) NATGatewayFullNatStateRefreshFunc(id string, field string, failStates []string) resource.StateRefreshFunc {
	return s.NATGatewayFullNatStateRefreshFuncWithApi(id, field, failStates, s.DescribeNATGatewayFullNat)
}

func (s *NATGatewayServiceV2) NATGatewayFullNatStateRefreshFuncWithApi(id string, field string, failStates []string, call func(id string) (map[string]interface{}, error)) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		object, err := call(id)
		if err != nil {
			if NotFoundError(err) {
...
// DescribeNATGatewayFullNat >>> Encapsulated.
