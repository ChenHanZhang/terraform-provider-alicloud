				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"acr_instance_id": {
							Type:       schema.TypeString,
							Optional:   true,
							Deprecated: "Field 'acr_instance_id' has been deprecated from provider version 1.266.0. ACR Enterprise version Image Repository ID, which must be entered when using ACR Enterprise version image. (Obsolete)",
						},
						"acceleration_info": {
							Type:       schema.TypeList,
							Computed:   true,
							Deprecated: "Field 'acceleration_info' has been deprecated from provider version 1.266.0. Image Acceleration Information (Obsolete)",
							MaxItems:   1,
							Elem: &schema.Resource{
								Schema: map[string]*schema.Schema{
									"status": {
										Type:       schema.TypeString,
										Computed:   true,
										Deprecated: "Field 'status' has been deprecated from provider version 1.266.0. Image Acceleration Status (Deprecated)",
									},
								},
							},
						},
						...
						"acceleration_type": {
							Type:         schema.TypeString,
							Optional:     true,
							Deprecated:   "Field 'acceleration_type' has been deprecated from provider version 1.266.0. Whether to enable Image acceleration. Default: The Default value, indicating that image acceleration is enabled. None: indicates that image acceleration is disabled. (Obsolete)",
...
func resourceAliCloudFcv3FunctionUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	fcv3ServiceV2 := Fcv3ServiceV2{client}
	objectRaw, _ := fcv3ServiceV2.DescribeFcv3Function(d.Id())

	if d.HasChange("invocation_restriction.0.disable") {
		var err error
		target := d.Get("invocation_restriction.0.disable").(bool)

		currentStatus, err := jsonpath.Get("$.invocationRestriction.disable", objectRaw)
		if err != nil {
			return WrapErrorf(err, FailedGetAttributeMsg, d.Id(), "$.invocationRestriction.disable", objectRaw)
		}
	...
	if d.HasChange("function_arn") {
		update = true
	}
	if v, ok := d.GetOk("function_arn"); ok || d.HasChange("function_arn") {
	...
	if d.HasChange("function_arn") {
		...
		if v, ok := d.GetOk("function_arn"); ok || d.HasChange("function_arn") {
			...
			query["ResourceId"] = resourceIdMapsArray
		}
	}

	query["ResourceType"] = StringPointer("function")
	body = request
	if update {
		wait := incrementalWait(3*time.Second, 5*time.Second)
		err = resource.Retry(d.Timeout(schema.TimeoutUpdate), func() *resource.RetryError {
