...
func resourceAliCloudAliKafkaScheduledScalingRuleCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return WrapErrorf(err, DefaultErrorMsg, "alicloud_ali_kafka_scheduled_scaling_rule", action, AlibabaCloudSdkGoERROR)
	}
...
func resourceAliCloudAliKafkaScheduledScalingRuleRead(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		if !d.IsNewResource() && NotFoundError(err) {
			log.Printf("[DEBUG] Resource alicloud_ali_kafka_scheduled_scaling_rule DescribeAliKafkaScheduledScalingRule Failed!!! %s", err)
			d.SetId("")
			return nil
		}
		return WrapError(err)
