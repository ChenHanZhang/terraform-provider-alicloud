...
func (s *AliKafkaServiceV2) DescribeConsumerGroupGetConsumerProgress(id string) (object map[string]interface{}, err error) {
	...
	v, err := jsonpath.Get("$.ConsumerProgress.TopicList.TopicList[*].OffsetList", response)
	if err != nil {
		return object, WrapErrorf(err, FailedGetAttributeMsg, id, "$.ConsumerProgress.TopicList.TopicList[*].OffsetList", response)
	}
