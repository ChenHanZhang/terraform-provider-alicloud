...
func (s *ConfigServiceV2) DescribeConfigRule(id string) (object map[string]interface{}, err error) {
	...
	object0, err := s.describeConfigRuleGetConfigRuleApi(id)
	if err != nil {
		if NotFoundError(err) {
			log.Printf("[DEBUG] Resource alicloud_config_rule ConfigServiceV2.DescribeConfigRule Failed!!! %s", err)
...
func (s *ConfigServiceV2) describeConfigRuleGetConfigRuleApi(id string) (object map[string]interface{}, err error) {
	client := s.client
