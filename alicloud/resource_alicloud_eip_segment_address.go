	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"isp": {
				...
				Optional:     true,
				ForceNew:     true,
				ValidateFunc: StringInSlice([]string{"BGP", "BGP_PRO", "ChinaTelecom", "ChinaUnicom", "ChinaMobile", "ChinaTelecom_L2", "ChinaUnicom_L2", "ChinaMobile_L2"}, false),
			},
			"netmode": {
				Type:     schema.TypeString,
				Optional: true,
			...
			"resource_group_id": {
				...
				Computed:     true,
				ForceNew:     true,
				ValidateFunc: StringMatch(regexp.MustCompile("(.*)"), "The ID of the resource group."),
			},
			"status": {
				Type:     schema.TypeString,
				Computed: true,
