...
// DescribeThreatDetectionWebLockConfig <<< Encapsulated get interface for ThreatDetection WebLockConfig.

func (s *ThreatDetectionServiceV2) DescribeThreatDetectionWebLockConfig(id string) (object map[string]interface{}, err error) {
	client := s.client
	var request map[string]interface{}
	var response map[string]interface{}
	var query map[string]interface{}
	...
	if len(v.([]interface{})) == 0 {
		return object, WrapErrorf(NotFoundErr("WebLockConfig", id), NotFoundMsg, response)
	}

	return v.([]interface{})[0].(map[string]interface{}), nil
}

func (s *ThreatDetectionServiceV2) ThreatDetectionWebLockConfigStateRefreshFunc(id string, field string, failStates []string) resource.StateRefreshFunc {
	return s.ThreatDetectionWebLockConfigStateRefreshFuncWithApi(id, field, failStates, s.DescribeThreatDetectionWebLockConfig)
}

func (s *ThreatDetectionServiceV2) ThreatDetectionWebLockConfigStateRefreshFuncWithApi(id string, field string, failStates []string, call func(id string) (map[string]interface{}, error)) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		object, err := call(id)
		if err != nil {
			if NotFoundError(err) {
...
// DescribeThreatDetectionWebLockConfig >>> Encapsulated.
