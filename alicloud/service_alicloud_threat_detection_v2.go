...
// DescribeThreatDetectionVulAutoConfig <<< Encapsulated get interface for ThreatDetection VulAutoConfig.

func (s *ThreatDetectionServiceV2) DescribeThreatDetectionVulAutoConfig(id string) (object map[string]interface{}, err error) {
	client := s.client
	var request map[string]interface{}
	var response map[string]interface{}
	var query map[string]interface{}
	...
	v, err := jsonpath.Get("$.Data", response)
	if err != nil {
		return object, WrapErrorf(NotFoundErr("VulAutoConfig", id), NotFoundMsg, response)
	}

	currentStatus := v.(map[string]interface{})["ConfigId"]
	if currentStatus == nil {
		return object, WrapErrorf(NotFoundErr("VulAutoConfig", id), NotFoundMsg, response)
	}
...
func (s *ThreatDetectionServiceV2) ThreatDetectionVulAutoConfigStateRefreshFunc(id string, field string, failStates []string) resource.StateRefreshFunc {
	return s.ThreatDetectionVulAutoConfigStateRefreshFuncWithApi(id, field, failStates, s.DescribeThreatDetectionVulAutoConfig)
}

func (s *ThreatDetectionServiceV2) ThreatDetectionVulAutoConfigStateRefreshFuncWithApi(id string, field string, failStates []string, call func(id string) (map[string]interface{}, error)) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		object, err := call(id)
		if err != nil {
			if NotFoundError(err) {
...
// DescribeThreatDetectionVulAutoConfig >>> Encapsulated.
