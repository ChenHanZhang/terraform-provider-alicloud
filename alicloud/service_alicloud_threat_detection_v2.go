...
// DescribeThreatDetectionIncidentInvestigation <<< Encapsulated get interface for ThreatDetection IncidentInvestigation.

func (s *ThreatDetectionServiceV2) DescribeThreatDetectionIncidentInvestigation(id string) (object map[string]interface{}, err error) {
	client := s.client
	var request map[string]interface{}
	var response map[string]interface{}
	var query map[string]interface{}
	request = make(map[string]interface{})
	query = make(map[string]interface{})
	request["IncidentInvestigationId"] = id
	request["RegionId"] = client.RegionId
	action := "GetIncidentInvestigation"
	...
	if err != nil {
		if IsExpectedErrors(err, []string{"IdempotentParameterMismatch"}) {
			return object, WrapErrorf(NotFoundErr("IncidentInvestigation", id), NotFoundMsg, response)
		}
		return object, WrapErrorf(err, DefaultErrorMsg, id, action, AlibabaCloudSdkGoERROR)
	}

	v, err := jsonpath.Get("$.IncidentInvestigation", response)
	if err != nil {
		return object, WrapErrorf(err, FailedGetAttributeMsg, id, "$.IncidentInvestigation", response)
	}

	return v.(map[string]interface{}), nil
}

func (s *ThreatDetectionServiceV2) ThreatDetectionIncidentInvestigationStateRefreshFunc(id string, field string, failStates []string) resource.StateRefreshFunc {
	return s.ThreatDetectionIncidentInvestigationStateRefreshFuncWithApi(id, field, failStates, s.DescribeThreatDetectionIncidentInvestigation)
}

func (s *ThreatDetectionServiceV2) ThreatDetectionIncidentInvestigationStateRefreshFuncWithApi(id string, field string, failStates []string, call func(id string) (map[string]interface{}, error)) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		object, err := call(id)
		if err != nil {
			if NotFoundError(err) {
...
// DescribeThreatDetectionIncidentInvestigation >>> Encapsulated.
