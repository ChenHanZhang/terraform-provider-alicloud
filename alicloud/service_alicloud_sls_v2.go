...
// DescribeSlsIngestProcessor <<< Encapsulated get interface for Sls IngestProcessor.

func (s *SlsServiceV2) DescribeSlsIngestProcessor(id string) (object map[string]interface{}, err error) {
	client := s.client
	var request map[string]interface{}
	var response map[string]interface{}
	var query map[string]*string
	...
	wait := incrementalWait(3*time.Second, 5*time.Second)
	err = resource.Retry(1*time.Minute, func() *resource.RetryError {
		response, err = client.Do("Sls", roaParam("GET", "2020-12-30", "GetIngestProcessor", action), query, nil, nil, hostMap, true)
	...
	addDebug(action, response, request)
	if err != nil {
		if IsExpectedErrors(err, []string{"404", "ProcessorNotExist"}) {
			return object, WrapErrorf(NotFoundErr("IngestProcessor", id), NotFoundMsg, response)
		}
		return object, WrapErrorf(err, DefaultErrorMsg, id, action, AlibabaCloudSdkGoERROR)
	}
...
func (s *SlsServiceV2) SlsIngestProcessorStateRefreshFunc(id string, field string, failStates []string) resource.StateRefreshFunc {
	return s.SlsIngestProcessorStateRefreshFuncWithApi(id, field, failStates, s.DescribeSlsIngestProcessor)
}

func (s *SlsServiceV2) SlsIngestProcessorStateRefreshFuncWithApi(id string, field string, failStates []string, call func(id string) (map[string]interface{}, error)) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		object, err := call(id)
		if err != nil {
			if NotFoundError(err) {
...
// DescribeSlsIngestProcessor >>> Encapsulated.
