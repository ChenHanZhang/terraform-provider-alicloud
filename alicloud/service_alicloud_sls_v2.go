...
// DescribeSlsDashboard <<< Encapsulated get interface for Sls Dashboard.

func (s *SlsServiceV2) DescribeSlsDashboard(id string) (object map[string]interface{}, err error) {
	client := s.client
	var request map[string]interface{}
	var response map[string]interface{}
	var query map[string]*string
	...
	wait := incrementalWait(3*time.Second, 5*time.Second)
	err = resource.Retry(1*time.Minute, func() *resource.RetryError {
		response, err = client.Do("Sls", roaParam("GET", "2020-12-30", "GetDashboard", action), query, nil, nil, hostMap, true)
...
func (s *SlsServiceV2) SlsDashboardStateRefreshFunc(id string, field string, failStates []string) resource.StateRefreshFunc {
	return s.SlsDashboardStateRefreshFuncWithApi(id, field, failStates, s.DescribeSlsDashboard)
}

func (s *SlsServiceV2) SlsDashboardStateRefreshFuncWithApi(id string, field string, failStates []string, call func(id string) (map[string]interface{}, error)) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		object, err := call(id)
		if err != nil {
			if NotFoundError(err) {
...
// DescribeSlsDashboard >>> Encapsulated.
