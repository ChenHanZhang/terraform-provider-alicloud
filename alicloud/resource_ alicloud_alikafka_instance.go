...
if v := d.Get("serverless_config");  !IsNil(v)   {
    ...
    if err != nil {
        return WrapErrorf(err, DefaultErrorMsg, " alicloud_alikafka_instance", action, AlibabaCloudSdkGoERROR)
    }
...
request["InstanceId"] = v
    ...
    if err != nil {
		...
		alikafkaServiceV2 := AlikafkaServiceV2{client}
				...
				log.Printf("[DEBUG] Resource  alicloud_alikafka_instance DescribeAlikafkaInstance Failed!!! %s", err)
				d.SetId("")
				return nil
			}
					return WrapError(err)
...
func resourceAliCloudAlikafkaInstanceDelete(d *schema.ResourceData, meta interface{}) error {
        ...
        if v, ok := d.GetOkExists("paid_type"); ok {
            if InArray(fmt.Sprint(v), []string{  "0", "4" }) {
                enableDelete = false
                log.Printf("[WARN] Cannot destroy resource  alicloud_alikafka_instance which paid_type valued 0,4. Terraform will remove this resource from the state file, however resources may remain.")
            }
        }
                        if enableDelete {
