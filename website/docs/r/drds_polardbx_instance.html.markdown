...
* `description` - (Optional, ForceNew, Available since v1.267.0) Instance remarks
* `dn_class` - (Required, ForceNew) Storage node specifications.
* `dn_node_count` - (Required, Int) The number of storage nodes.
* `engine_version` - (Required, ForceNew, Available since v1.267.0) Engine Version
* `is_read_db_instance` - (Optional, Available since v1.267.0) Whether the instance is read-only.
  - `true`: Yes
  - `false`: No

-> **NOTE:** The parameter is immutable after resource creation. It only applies during resource creation and has no effect when modified post-creation.

* `primary_db_instance_name` - (Optional, Available since v1.267.0) If the instance is a read-only instance, you must specify the primary instance.
...
* `create` - (Defaults to 26 mins) Used when create the Polardbx Instance.
* `delete` - (Defaults to 120 mins) Used when delete the Polardbx Instance.
* `update` - (Defaults to 31 mins) Used when update the Polardbx Instance.
