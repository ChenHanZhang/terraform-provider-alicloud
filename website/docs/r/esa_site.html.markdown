...
* `ai_template` - (Optional, Computed, Available since v1.272.1) HTTP DDoS Intelligent Protection Level. Values:
  ...
  - `level60`: Normal.
  - `level90`: Strict.
* `automatic_frequency_control_action_type` - (Optional, Available since v1.274.0) Disposal action.Valid values:
  - `observe`: Observe.
  - `deny`: Block.
  - `js`: Js verification.
* `automatic_frequency_control_enable` - (Optional, Available since v1.274.0) Switch.Valid values:
  - `on`: on.
  - `off`: off.
* `automatic_frequency_control_level` - (Optional, Available since v1.274.0) Protection Level.Valid values:
  - `loose`: Loose.
  - `normal`: Normal.
  - `strict`: Strict.
