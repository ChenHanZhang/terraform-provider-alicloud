...
* `bursting_enabled` - (Optional, Computed, Available since v1.283.0) Whether to enable this function for disks that support Burst (performance Burst). Valid values:
true: Yes.
false: No.
Description
For disks that do not support the Burst function, passing in any value will report an error.
* `delete_with_instance` - (Optional, Computed, Available since v1.283.0) Specifies whether to release the disk together with the instance. Valid values:
...
* `disk_name` - (Optional, Computed) The name of the disk. It must be 2 to 128 characters in length and can contain letters defined in the Unicode standard (including English letters, Chinese characters, and digits). It can also include colons (:), underscores (_), periods (.), or hyphens (-).
...
* `instance_charge_type` - (Optional, ForceNew, Computed) The billing method. Valid values:
...
* `instance_id` - (Optional, ForceNew, Computed, Available since v1.283.0) The ID of the instance to which the disk is attached. If `InstanceChargeType` is `Prepaid`, you must specify the ID of a prepaid instance.  
* `performance_level` - (Optional, Computed) The performance level of the disk when it is an ESSD cloud disk. Valid values:
...
* `resource_group_id` - (Optional, Computed, Available since v1.283.0) The ID of the resource group to which the disk belongs.
...
* `snapshot_id` - (Optional, ForceNew) The snapshot used to create the disk.
...
* `create` - (Defaults to 10 mins) Used when create the Custom Disk.
* `delete` - (Defaults to 5 mins) Used when delete the Custom Disk.
* `update` - (Defaults to 31 mins) Used when update the Custom Disk.
