...
* `access_mode` - (Optional, ForceNew, Computed, Set, Available since v1.235.0) Configuration of the reverse access VPC-NAT mode. See [`access_mode`](#access_mode) below.
* `auto_pay` - (Optional, Available since v1.270.0) Subscription public NAT gateways are no longer available for new purchases. This parameter is no longer used.
...
* `force_delete` - (Optional, Available since v1.270.0) Specifies whether to forcibly delete the NAT gateway. Valid values:
  - `true`: Forces deletion. When this value is set to `true`, the following actions are performed automatically:
    ...
    - If the NAT gateway is associated with an EIP, the system automatically disassociates it.

    - If the NAT gateway has an undeleted NAT bandwidth package, the system automatically deletes it.
  - `false` (default): Does not force deletion. When this value is set to `false`, you must perform the following actions manually before deletion:

    - Delete any existing NAT bandwidth packages associated with the NAT gateway.

    - Delete all SNAT entries configured on the NAT gateway.

    - Delete all DNAT entries configured on the NAT gateway.

    - Disassociate any EIPs bound to the NAT gateway.

-> **NOTE:** This parameter only takes effect when deletion is triggered.

* `icmp_reply_enabled` - (Optional, Computed, Available since v1.235.0) Specifies whether to enable ICMP reply. Valid values:
...
* `nat_gateway_private_info` - (Required, ForceNew, Set, Available since v1.270.0) Private IP address information of the NAT gateway.   See [`nat_gateway_private_info`](#nat_gateway_private_info) below.
* `nat_type` - (Required, ForceNew, Available since v1.102.0) The type of the public NAT gateway. The current value is `Enhanced`, which indicates an enhanced NAT gateway.
...
* `spec` - (Optional, ForceNew) The specification of the Internet NAT gateway instance. When `InternetChargeType` is `PayBySpec`, the following values are returned:
  ...
  - `Middle`: Medium.
  - `Large`: Large.
When `InternetChargeType` is `PayByLcu`, this parameter returns an empty value.
...
* `mode_value` - (Optional, ForceNew, Computed) The access mode. Valid values:
  - `route`: Route mode.
  - `tunnel`: Tunnel mode.

-> **NOTE:**  When this parameter is specified, the `PrivateLinkEnabled` parameter must be set to `true`.

* `tunnel_type` - (Optional, ForceNew, Computed) The tunnel mode type:
  - `geneve`: Geneve type.

-> **NOTE:**  This parameter is valid only when the access mode is set to tunnel mode.
...
When you create a NAT gateway, you must specify the vSwitch to which the NAT gateway belongs. The system assigns an available private IP address from the vSwitch to the NAT gateway.
  - If you want to create a NAT gateway in an existing vSwitch, ensure that the zone where the vSwitch is deployed supports NAT gateway creation and that the vSwitch has available IP addresses.
  - If you have not created a vSwitch, first create one in a zone that supports NAT gateway creation, and then specify the vSwitch for the NAT gateway.
...
* `nat_gateway_private_info` - Private IP address information of the NAT gateway.
  ...
  * `iz_no` - The zone to which the NAT gateway instance belongs.
...
* `region_id` - The region ID of the NAT gateway.
