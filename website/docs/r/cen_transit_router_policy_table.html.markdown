...
* `description` - (Optional) The description of the policy-based route table instance.
* `dry_run` - (Optional) - `false` (default): Sends a normal request. After passing the check, the operation is executed directly.
  - `true`: Sends a dry-run request to perform only validation without executing the operation. If validation fails, an error is returned. If validation succeeds, the error code `DryRunOperation` is returned.
...
* `name` - (Optional) The name of the policy-based route table instance.
* `tags` - (Optional, Map) The list of tags bound to the policy-based route table instance.
* `transit_router_id` - (Required, ForceNew) The ID of the Enterprise Edition transit router instance.
...
* `status` - The status of the policy-based route table instance.
