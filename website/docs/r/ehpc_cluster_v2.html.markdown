...
resource "alicloud_vpc" "full_example_vpc" {
  is_default = false
  cidr_block = "10.0.0.0/24"
  vpc_name   = "example-cluster-vpc"
}

resource "alicloud_nas_access_group" "full_example_access_group" {
  access_group_type = "Vpc"
  description       = "挂载点创建example"
  access_group_name = "StandardMountTarget"
  file_system_type  = "standard"
}

resource "alicloud_nas_file_system" "full_example_nas" {
...
resource "alicloud_vswitch" "full_example_vswitch" {
  is_default   = false
  vpc_id       = alicloud_vpc.full_example_vpc.id
  zone_id      = "cn-hangzhou-k"
  cidr_block   = "10.0.0.0/24"
  vswitch_name = "example-cluster-vsw"
}

resource "alicloud_nas_access_rule" "full_example_access_rule" {
  priority          = "1"
  access_group_name = alicloud_nas_access_group.full_example_access_group.access_group_name
  file_system_type  = alicloud_nas_file_system.full_example_nas.file_system_type
  source_cidr_ip    = "10.0.0.0/24"
}

resource "alicloud_nas_mount_target" "full_example_mount_domain" {
  vpc_id            = alicloud_vpc.full_example_vpc.id
  network_type      = "Vpc"
  access_group_name = alicloud_nas_access_group.full_example_access_group.access_group_name
  vswitch_id        = alicloud_vswitch.full_example_vswitch.id
  file_system_id    = alicloud_nas_file_system.full_example_nas.id
}

resource "alicloud_security_group" "full_example_security_group" {
  vpc_id              = alicloud_vpc.full_example_vpc.id
...
resource "alicloud_ehpc_cluster_v2" "default" {
  ...
  cluster_vpc_id    = alicloud_vpc.full_example_vpc.id
  cluster_category  = "Standard"
  cluster_mode      = "Integrated"
  security_group_id = alicloud_security_group.full_example_security_group.id
  addons {
    version        = "1.0"
    services_spec  = <<EOF
[
    ...
    resources_spec = <<EOF
{
  ...
  cluster_name        = "full-example-cluster"
  deletion_protection = true
  shared_storages {
    mount_directory     = "/home"
    nas_directory       = "/"
    mount_target_domain = alicloud_nas_mount_target.full_example_mount_domain.mount_target_domain
    protocol_type       = "NFS"
    file_system_id      = alicloud_nas_file_system.full_example_nas.id
  ...
  cluster_vswitch_id = alicloud_vswitch.full_example_vswitch.id
  manager {
    manager_node {
      system_disk {
        category = "cloud_essd"
...
* `client_version` - (Optional) Specifies whether to enable auto scale-out for the cluster. Valid values:
...
* `auto_renew` - (Optional) Whether to automatically renew. This parameter takes effect only when the value of InstanceChargeType is PrePaid. Value range:
  - true: Automatic renewal.
  - false: Do not renew automatically (default).
* `auto_renew_period` - (Optional, Int) The renewal duration of a single automatic renewal. Value range:
  - When PeriodUnit = Week: 1, 2, 3.
  - When PeriodUnit = Month: 1, 2, 3, 6, 12, 24, 36, 48, 60.

Default value: 1.
* `duration` - (Optional, Int) The duration of the preemptible instance, in hours. Value:
...
* `period` - (Optional, Int) The duration of the resource purchase. The unit is specified by PeriodUnit. The parameter InstanceChargeType takes effect only when the value is PrePaid and is a required value. Once DedicatedHostId is specified, the value range cannot exceed the subscription duration of the DDH. Value range:
...
* `period_unit` - (Optional) The unit of duration of the year-to-month billing method. Value range:
  - Week.
  - Month (default).
* `spot_price_limit` - (Optional, Float) Set the maximum price per hour for the instance. The maximum number of decimals is 3. It takes effect when the value of the SpotStrategy parameter is SpotWithPriceLimit.
* `spot_strategy` - (Optional) The bidding strategy for pay-as-you-go instances. This parameter takes effect when the value of the InstanceChargeType parameter is PostPaid. Value range:
...
* `system_disk` - (Optional, List) System disk configuration of the management node. See [`system_disk`](#manager-manager_node-system_disk) below.
...
* `category` - (Optional) Manage the system disk configuration of the node. Value range:
  - cloud_efficiency: The Ultra cloud disk.
  - cloud_ssd:SSD cloud disk.
  - cloud_essd:ESSD cloud disk.
  - cloud: ordinary cloud disk.
* `level` - (Optional) When creating an ESSD cloud disk to use as a system disk, set the performance level of the cloud disk. Value range:
...
* `size` - (Optional, Int) The system disk size of the management node. Unit: GiB. Value range:
