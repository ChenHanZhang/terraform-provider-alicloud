...
* `sequence` - (Optional, Int) The order in which the rule is executed. A smaller value gives priority to the rule.
...
* `site_version` - (Optional, ForceNew, Computed, Int) The version number of the website configurations. You can use this parameter to specify a version of your website to apply the feature settings. By default, version 0 is used.
