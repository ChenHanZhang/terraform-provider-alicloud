...
Provides a Cloud Firewall Nat Firewall Control Policy resource.

Nat firewall access control policy.
...
Value:
  ...
  - `drop`: Refused
  - `log`: Observation
* `application_name_list` - (Required, List) The list of application types supported by the access control policy.
* `description` - (Required) The description of the access control policy.
* `dest_port` - (Optional, Computed) The destination port of traffic access in the access control policy. Value:
...
* `dest_port_type` - (Optional, Computed) The destination port type of the access traffic in the security access control policy.
  - `port`: port
...
* `domain_resolve_type` - (Optional, Int) The domain name resolution method of the access control policy. The policy is enabled by default after it is created. Value:
  - `0`: Based on FQDN
  - `1`: DNS-based dynamic resolution
  - `2`: dynamic resolution based on FQDN and DNS
* `end_time` - (Optional, Int) The end time of the policy validity period of the access control policy. Expresses using the second-level timestamp format. Must be full or half time and at least half an hour greater than the start time.
...
* `new_order` - (Required, Available since v1.276.0, Deprecated since v1.286.0) The priority for the access control policy to take effect. The priority number increases sequentially from 1, and the smaller the priority number, the higher the priority.
...
* `repeat_days` - (Optional, List) Collection of recurring dates for the policy validity period of the access control policy.
...
* `repeat_type` - (Optional, Computed) The type of repetition for the policy validity period of the access control policy. Value:
...
* `start_time` - (Optional, Int) The start time of the policy validity period of the access control policy. Expresses using the second-level timestamp format. It must be a full or half hour and at least half an hour less than the end time.
...
* `destination_group_cidrs` - The list of network segments in the destination address book in the access control policy.
* `destination_group_type` - The destination address book type in the access control policy.
* `dns_result` - DNS resolution result.
* `dns_result_time` - The timestamp of the DNS resolution.
* `hit_last_time` - The timestamp of the latest hit.
* `hit_times` - The number of hits to the access control policy.
* `modify_time` - The time when the policy was modified.
* `source_group_cidrs` - The list of CIDR blocks in the source address book in the access control policy.
* `source_group_type` - The source address book type in the access control policy.
* `spread_cnt` - The number of specifications used by the access control policy, that is, the cumulative value of the number of specifications used by each policy.
