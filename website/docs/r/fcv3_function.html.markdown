...
* `instance_isolation_mode` - (Optional, Computed) Instance isolation mode
...
* `invocation_restriction` - (Optional, List) Invocation Restriction Detail See [`invocation_restriction`](#invocation_restriction) below.
...
* `acceleration_type` - (Optional, Deprecated since v1.266.0) Whether to enable Image acceleration. Default: The Default value, indicating that image acceleration is enabled. None: indicates that image acceleration is disabled. (Obsolete)
* `acr_instance_id` - (Optional, Deprecated since v1.266.0) ACR Enterprise version Image Repository ID, which must be entered when using ACR Enterprise version image. (Obsolete)
...
* `command` - (Optional, List) Lifecycle Initialization Phase Callback Instructions
...
* `disable` - (Optional) Whether invocation is disabled
