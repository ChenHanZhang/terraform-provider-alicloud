...
resource "alicloud_alikafka_sasl_user" "defaulty2f9ds" {
  type        = "scram"
  username    = "qwoeiuqwoieurandom600"
  password    = "123123"
...
* `acl_operation_type` - (Required, ForceNew) The operation type. Return value:
  - `Write`: Write
  - `Read`: Read

  - `Describe`: read TransactionalId
  - `IdempotentWrite`: idempotent Cluster. The Serverless instance does not support this value.
  - `IDEMPOTENT_WRITE`: idempotent Cluster. Only Serverless instances are available.
  - `DESCRIBE_CONFIGS`: queries the configuration. Only the Serverless instance is available.
...
* `id` - The ID of the resource supplied above.The value is formulated as `<instance_id>:<username>:<acl_resource_name>:<acl_resource_type>:<acl_operation_type>:<acl_resource_pattern_type>`.
...
```shell
$ terraform import alicloud_ali_kafka_acl.example <instance_id>:<username>:<acl_resource_name>:<acl_resource_type>:<acl_operation_type>:<acl_resource_pattern_type>
```