...
Default value: empty.  
* `disk_id` - (Required, ForceNew) The ID of the specified disk device.
...
* `retention_days` - (Optional, Computed, Int) Number of days to retain the snapshot. The retention period starts from the snapshot's creation time (CreationTime). After a standard snapshot is created, it must be retained for at least 14 days before it can be archived.
...
* `snapshot_id` - (Required, ForceNew, Available since v1.272.0) Snapshot identifier. The value can be a JSON array consisting of multiple snapshot IDs, with a maximum of 100 IDs. Separate multiple IDs with commas (,).
