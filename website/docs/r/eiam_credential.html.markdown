...
A credential used to authenticate the associated principal. Such credentials are typically long-term, support version management, and require rotation. Examples include passwords for system users on Linux hosts and public-private key pairs.  .
...
* `credential_external_id` - (Optional, ForceNew) The external unique identifier of the credential.  
...
* `credential_sharing_scope` - (Optional, ForceNew, Computed) The sharing scope of the credential.  
...
* `exclusive_user_id` - (Optional, ForceNew) Exclusive user ID of the credential.  
