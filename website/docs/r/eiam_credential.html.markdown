...
Credentials used to authenticate associated principals. These credentials are typically long-term, support version management, and require rotation. Examples include passwords for system users on Linux hosts and public-private key pairs.  .
...
* `credential_subject_id` - (Optional, ForceNew) Subject ID to which the credential belongs.  
