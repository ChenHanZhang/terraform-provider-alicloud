...
* `backend_node_groups` - (Optional, List) BackendNodeGroups See [`backend_node_groups`](#backend_node_groups) below.
...
* `frontend_node_groups` - (Optional, List) FrontendNodeGroups See [`frontend_node_groups`](#frontend_node_groups) below.

-> **NOTE:** This parameter is immutable. Changing it after creation has no effect.

* `instance_name` - (Required) The name of the instance.
* `kms_key_id` - (Optional, ForceNew) KmsKeyId
* `observer_node_groups` - (Optional, List) ObserverNodeGroups See [`observer_node_groups`](#observer_node_groups) below.
...
* `cu` - (Optional, Int) Number of CUs. CU (Compute Unit) is the basic measurement unit of the service, where 1 CU = 1 CPU core + 4 GiB memory.
* `disk_number` - (Optional, Int) The number of disks.
* `local_storage_instance_type` - (Optional) Local SSD instance specifications.
* `resident_node_number` - (Optional, Int) Resident node number of node group.
* `spec_type` - (Optional) Compute group specification types include the following:
  - standard
  - localSSD
  - bigData
  - ramEnhanced
  - networkEnhanced
* `storage_performance_level` - (Optional) Performance levels of cloud disks include the following values:
...
* `cu` - (Optional, Int) Number of CUs. CU (Compute Unit) is the basic measurement unit of the service, where 1 CU = 1 CPU core + 4 GiB memory.
* `disk_number` - (Optional, Int) DiskNumber
* `local_storage_instance_type` - (Optional) Local SSD instance specifications.
* `resident_node_number` - (Optional, Int) Resident node number of node group.
* `spec_type` - (Optional) Compute group specification types include the following:
  - standard
...
```shell
$ terraform import alicloud_star_rocks_instance.example <instance_id>
```