...
* `create_time` - The timestamp when the service trail was created.
* `service_trail` - The configuration information of the service trail.
  * `config` - The status of the service trail.
  * `update_time` - The timestamp of the most recent service update, in milliseconds.
...
```shell
$ terraform import alicloud_threat_detection_sas_trail.example <Alibaba Cloud Account ID>
```