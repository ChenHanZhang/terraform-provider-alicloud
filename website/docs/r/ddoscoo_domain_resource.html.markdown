...
* `ai_mode` - (Optional, Computed, Available since v1.270.0) the mode of the Intelligent Protection policy.
  - watch: the Warning mode
  - defense: the Defense mode
* `ai_template` - (Optional, Computed, Available since v1.270.0) the level of the Intelligent Protection policy.
...
* `bw_list_enable` - (Optional, Computed, Int, Available since v1.270.0) The status of the blacklist and whitelist feature. Valid values:
  - 0: Disabled
  - 1: Enabled
* `cc_global_switch` - (Optional, Computed, Available since v1.270.0) CC safety protection switch.
  - 0: Disabled
  - 1: Enabled
...
* `custom_headers` - (Optional, Computed, JsonString, Available since v1.261.0) The key-value pair of the custom header. The key specifies the header name, and the value specifies the header value. You can specify up to five key-value pairs. The key-value pairs can be up to 200 characters in length.
...
* `instance_ids` - (Required, List) InstanceIds
...
* `ocsp_enabled` - (Optional, Available since v1.208.0) Specifies whether to enable the OCSP feature. Valid values:
  - `True`: Opened
  - `False`: Not enabled
* `proxy_types` - (Required, List) Protocol type and port number information. See [`proxy_types`](#proxy_types) below.
* `real_servers` - (Required, List, Available since v1.231.0) Server address information of the source station.
...
* `white_list` - (Optional, List) IP whitelist list.
