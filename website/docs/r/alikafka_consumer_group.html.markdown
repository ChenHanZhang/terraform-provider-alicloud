...
* `offset` - (Optional, Int, Available since v1.267.0) Partition site.
* `partition` - (Optional, Int, Available since v1.267.0) The partition ID.
* `remark` - (Optional, ForceNew, Available since v1.267.0) Remarks.
* `reset_type` - (Optional, Available since v1.267.0) The type of the consumer group's consumption point. The following two types are supported:
  - `timestamp` (default)
  - `offset`
...
* `time` - (Optional, Available since v1.267.0) The input time parameter, in Unix timestamp format, in milliseconds.
...
* `topic` - (Optional, ForceNew, Available since v1.267.0) The Topic to which the Consumer subscribes
