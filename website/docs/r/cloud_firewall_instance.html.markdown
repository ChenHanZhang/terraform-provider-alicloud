...
* `auto_asset_protection` - (Optional, Computed, Available since v1.282.0) Internet asset protection switch. Valid values:
  - `true`: Enabled.
  - `false`: Disabled.
* `band_width` - (Optional, Int) Public network traffic processing capacity.
...
* `cfw_log` - (Optional, Computed) Log delivery status. Valid values:
  - `true`: Enabled
  - `false`: Disabled.
* `fw_vpc_number` - (Optional, ForceNew, Computed, Int) This property does not have a description in the spec, please add it before generating code.
* `ip_number` - (Optional, ForceNew, Computed, Int) This property does not have a description in the spec, please add it before generating code.
...
* `product_code` - (Required, ForceNew, Available since v1.282.0) The product code. You can query product codes by calling the `QueryProductList` operation.
* `product_type` - (Required, ForceNew, Available since v1.282.0) The product type.
...
* `sdl` - (Optional, Available since v1.282.0) Data leakage protection activation status.
