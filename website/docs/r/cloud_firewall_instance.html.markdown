...
* `auto_asset_protection` - (Optional, Computed, Available since v1.274.0) Internet asset protection switch. Valid values:
  - `true`: Enabled.
  - `false`: Disabled.
...
* `product_code` - (Required, ForceNew, Available since v1.274.0) The product code. You can query product codes by calling the `QueryProductList` operation.
* `product_type` - (Required, ForceNew, Available since v1.274.0) The product type.
...
* `sdl` - (Optional, Available since v1.274.0) Data leakage protection activation status.
