...
* `notification_policy` - (Optional, Set) The notification policy. See [`notification_policy`](#notification_policy) below.
...
The notification_policy supports the following:
* `region` - (Optional) Notification policy region.
* `workspace` - (Optional) Notification policy workspace name.
