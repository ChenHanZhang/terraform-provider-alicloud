...
* `composite_expression` - (Optional, ForceNew, Set) Alert condition for multiple metrics.
...
* `escalations` - (Optional, ForceNew, Set) The trigger conditions for alert levels. See [`escalations`](#escalations) below.
...
* `labels` - (Optional, List) When a metric meets the alert condition and an alert is triggered, the labels are written to the metric and displayed in the alert notification.
...
* `send_ok` - (Optional) Specifies whether to send recovery notifications.
...
* `status` - (Optional, Computed) The enabled status of the alarm rule. Valid values:
  - true: enabled.
  - false: disabled.
* `targets` - (Optional, List) None. See [`targets`](#targets) below.
...
* `expression_list` - (Optional, List) A list of alert conditions created using standard expressions. See [`expression_list`](#composite_expression-expression_list) below.
...
* `critical` - (Optional, ForceNew, Set) The trigger condition for Critical-level alerts. See [`critical`](#escalations-critical) below.
* `info` - (Optional, ForceNew, Set) Trigger conditions for Info-level alerts. See [`info`](#escalations-info) below.
* `warn` - (Optional, ForceNew, Set) Trigger condition for Warn-level alerts.   See [`warn`](#escalations-warn) below.
...
* `annotations` - (Optional, List) When a Prometheus alert is triggered, the key-value pairs of annotations are rendered to help you better understand the metric or alert rule.
...
* `arn` - (Required) Resource ARN. The format is `acs:{cloud-service-abbreviation}:{regionId}:{userId}:/{resource-type}/{resource-name}/message`. Example: `acs:mns:cn-hangzhou:120886317861****:/queues/test123/message`. The parameters are described as follows:
  - {cloud-service-abbreviation}: Only Message Service (MNS) is currently supported.
  - {userId}: Alibaba Cloud account ID.
  - {regionId}: The region where the message queue or topic resides.
  - {resource-type}: The type of resource that receives the alert. Valid values:
...
* `json_params` - (Optional) JSON-formatted parameters for the alert callback.
* `level` - (Optional) Alert severity level. Valid values:
  - INFO: Information
  - WARN: Warning
  - CRITICAL: Critical
* `target_id` - (Required) Alert trigger target ID.
