...
* `auto_mode` - (Optional, ForceNew, Computed, List, Available since v1.265.0) Whether to enable auto mode. See [`auto_mode`](#auto_mode) below.
...
* `data_disks` - (Optional, Computed, List) Configure the data disk of the node in the node pool. See [`data_disks`](#data_disks) below.
...
* `instance_metadata_options` - (Optional, ForceNew, Computed, List, Available since v1.265.0) ECS instance metadata access configuration. See [`instance_metadata_options`](#instance_metadata_options) below.
* `instance_patterns` - (Optional, List, Available since v1.265.0) Instance property configuration. See [`instance_patterns`](#instance_patterns) below.
...
* `on_demand_base_capacity` - (Optional, Computed) The minimum number of pay-as-you-go instances that must be kept in the scaling group. Valid values: 0 to 1000. If the number of pay-as-you-go instances is less than the value of this parameter, Auto Scaling preferably creates pay-as-you-go instances.
* `on_demand_percentage_above_base_capacity` - (Optional, Computed) The percentage of pay-as-you-go instances among the extra instances that exceed the number specified by `on_demand_base_capacity`. Valid values: 0 to 100.
...
* `system_disk_encrypt_algorithm` - (Optional, Computed) The encryption algorithm used by the system disk. Value range: aes-256.
...
* `system_disk_size` - (Optional, Computed, Int) The size of the system disk. Unit: GiB. The value of this parameter must be at least 1 and greater than or equal to the image size. Default value: 40 or the size of the image, whichever is larger.
...
* `enabled` - (Optional, ForceNew, Available since v1.265.0) Whether to enable auto mode.
...
* `size` - (Optional, Computed, Int) The size of a data disk, Its valid value range [40~32768] in GB. Default to `40`.
...
* `http_tokens` - (Optional, ForceNew, Available since v1.265.0) ECS instance metadata access mode configuration. Value range:
...
* `cores` - (Optional, Int, Available since v1.265.0) The number of vCPU cores of the instance type. Example value: 8.
* `cpu_architectures` - (Optional, List, Available since v1.265.0) The CPU architecture of the instance. Value range:
  - X86
  - ARM
* `excluded_instance_types` - (Optional, List, Available since v1.265.0) Instance specifications to be excluded. You can exclude individual specifications or entire specification families by using the wildcard character (*). For example:
...
* `instance_categories` - (Optional, List, Available since v1.265.0) Instance classification. Value range:
...
* `instance_family_level` - (Optional, Available since v1.265.0) Instance specification family level, value range:
  ...
  - CreditEntryLevel: integral entry level, that is, burst performance instance. CPU credits are used to ensure computing performance, which is suitable for business scenarios with low CPU usage and occasional sudden CPU usage. For more information, see Burst Performance Instance Overview.
* `instance_type_families` - (Optional, List, Available since v1.265.0) Specifies the instance type family. Example value: ecs.g6
* `max_cpu_cores` - (Optional, Int, Available since v1.265.0) The maximum number of vCPU cores of the instance type. Example value: 8. MaxCpuCores cannot exceed 4 times of MinCpuCores.
* `max_memory_size` - (Optional, Float, Available since v1.265.0) The maximum memory of the instance type. Unit: GiB, example value: 8,MaxMemoryCores does not support more than 4 times MinMemoryCores.
* `memory` - (Optional, Float, Available since v1.265.0) The memory size of the instance type, in GiB. Example value: 8.
* `min_cpu_cores` - (Optional, Int, Available since v1.265.0) The minimum number of vCPU cores of the instance type. Example value: 4. MaxCpuCores cannot exceed 4 times of MinCpuCores.
* `min_memory_size` - (Optional, Float, Available since v1.265.0) The minimum memory of the instance type. Unit: GiB, example value: 4,MaxMemoryCores does not support more than 4 times MinMemoryCores.
...
* `server_tls_bootstrap` - (Optional, Available since v1.265.0) Used to enable the kubelet server certificate signing and rotation via CSR.
