...
* `cis_enabled` - (Optional, ForceNew) Whether to enable CIS reinforcement.
...
* `data_disks` - (Optional) Configure the data disk of the node in the node pool. See [`data_disks`](#data_disks) below.
...
* `instance_charge_type` - (Optional, Computed) Paying type of the node pool node. Default value: 'PostPaid '. Valid values:
  - `PrePaid` : Prepaid.
  - `PostPaid` : Pay by volume.
* `instance_types` - (Required) In the node instance specification list, you can select multiple instance specifications as alternatives. When each node is created, it will try to purchase from the first specification until it is created successfully. The final purchased instance specifications may vary with inventory changes.
* `internet_charge_type` - (Optional) The public IP charging type. Value:
  - PayByBandwidth: Billing for fixed bandwidth.
  - PayByTraffic: Billing by usage flow.
* `internet_max_bandwidth_out` - (Optional) The maximum bandwidth of the public IP address of the node. The unit is Mbps(Mega bit per second). The value range is:\[1,100\].
* `key_name` - (Optional) The name of the key pair, and 'login_password. When the node pool is a managed version node pool, only 'key_pair 'is supported'.
* `kubelet_configuration` - (Optional) Supported kubelet configurations. See [`kubelet_configuration`](#kubelet_configuration) below.
* `labels` - (Optional) Node labels to add labels to Kubernetes cluster nodes. See [`labels`](#labels) below.
* `login_as_non_root` - (Optional, ForceNew) Whether the ECS instance is logged on as a ecs-user user. Valid value: `true` and `false`.
* `management` - (Optional, Computed) Managed node pool configuration. See [`management`](#management) below.
...
* `node_name_mode` - (Optional, ForceNew, Computed) Each node name consists of a prefix, its private network IP, and a suffix, the input format is `customized,,ip,`.
...
The parameter contains four parts, separated by commas. For example, if the string "customized,aliyun,ip,com" is passed in (where "customized" and "ip" are fixed strings, aliyun is the prefix, and com is the suffix), the name of the node is aliyun.192.168.xxx.xxx.com.
...
* `password` - (Optional) SSH login password, and 'key_pair. The password rule is 8 to 30 characters and contains at least three items (upper and lower case letters, numbers, and special symbols). For security reasons, the password query results are encrypted.
* `period` - (Optional) The value of 'instance_charge_type' is mandatory and takes effect only when the value of 'instance_charge_type' is set to 'PrePaid'. The value range of 'period_unit' is set to Month, and the value range of 'period' is set to {1, 2, 3,6,12}.  Default value: 1.
* `period_unit` - (Optional) The payment period for nodes in the node pool. If 'PrePaid' is specified, the payment period must be specified. Month: The unit of time is Month.
* `platform` - (Optional, Computed, Deprecated since v1.216.0) Operating system release. Default value: 'AliyunLinux '. Valid values: `CentOS`, `AliyunLinux`, `Windows`, `WindowsCore`.
* `private_pool_options` - (Optional) Private node pool configuration. See [`private_pool_options`](#private_pool_options) below.
* `rds_instances` - (Optional) The list of RDS instances.
* `resource_group_id` - (Optional, Computed) The ID of the resource group.
* `rolling_policy` - (Optional) Rotary configuration. See [`rolling_policy`](#rolling_policy) below.
* `runtime_name` - (Optional, Computed) The Container Runtime.
* `runtime_version` - (Optional, Computed) The container runtime version. .
* `scaling_config` - (Optional, Computed) Automatic scaling configuration. See [`scaling_config`](#scaling_config) below.
* `scaling_policy` - (Optional, Computed) Scaling group mode, default value: 'relase '. Valid values:
...
* `security_group_id` - (Optional, ForceNew, Computed, Deprecated since v1.216.0) The security group ID of the node pool. Select one of 'security_group_ids 'and 'security_group_ids'. Use 'security_group_ids 'recommend'.
* `security_group_ids` - (Optional, ForceNew, Computed) In the security group ID list, select 'security_group_ids 'recommend 'security_group_ids'. When both 'security_group_ids 'and 'security_group_ids' are specified, 'security_group_ids 'is used first'.
* `soc_enabled` - (Optional, ForceNew) Whether to enable SOC.
...
* `system_disk_bursting_enabled` - (Optional) Specifies whether to enable the burst feature for system disks. Valid values:`true`: enables the burst feature. `false`: disables the burst feature. This parameter is supported only when SystemDiskCategory is set to cloud_auto.
* `system_disk_categories` - (Optional, Computed) The multi-disk categories of the system disk. When a high-priority disk type cannot be used, Auto Scaling automatically tries to create a system disk with the next priority disk category. Valid values: `cloud`: cloud disk. `cloud_efficiency`: a high-efficiency cloud disk. `cloud_ssd`:SSD cloud disk. `cloud_essd`: ESSD cloud disk.
* `system_disk_category` - (Optional, Computed) Node system disk type. Valid values:
  - `Cloud_efficiency` : the high-efficiency cloud disk.
...
* `system_disk_performance_level` - (Optional) The disk performance of the system disk of the node takes effect only for the ESSD disk.
  - `PL0` : concurrency limit I/O performance is medium, read and write delay is relatively stable.
  - `PL1` : concurrency limit I/O performance is medium, read and write delay is relatively stable.
  - `PL2` : concurrency limit I/O performance is high, read and write delay is stable.
  - `PL3` : concurrency limit I/O performance is extremely high, read and write delay is extremely stable.
* `system_disk_provisioned_iops` - (Optional) The predefined IOPS of a system disk. Valid values: 0 to min{50,000, 1,000 × Capacity - Baseline IOPS}. Baseline IOPS = min{1,800 + 50 × Capacity, 50,000}. This parameter is supported only when `SystemDiskCategory` is set to `cloud_auto`.
* `system_disk_size` - (Optional) The system disk size of the node. Unit: GiB.  Value range:\[20,500\].
...
* `taints` - (Optional) Stain configuration. See [`taints`](#taints) below.
...
* `vswitch_ids` - (Required) The ID of the VSwitch. Valid values:\[1,20\].
-> **NOTE:**  To ensure high availability, we recommend that you select virtual switches in different zones.
...
* `name` - (Deprecated since v1.216.0). Field 'name' has been deprecated from provider version 1.216.0. New field 'node_pool_name' instead.
...
* `name` - (Optional) The length is 2~128 English or Chinese characters. It must start with an uppercase or lowr letter or a Chinese character and cannot start with http:// or https. Can contain numbers, colons (:), underscores (_), or dashes (-).
...
* `provisioned_iops` - (Optional) The read/write IOPS preconfigured for the data disk, which is configured when the disk type is cloud_auto.
...
* `allowed_unsafe_sysctls` - (Optional) Allowed sysctl mode whitelist.
...
* `auto_repair_policy` - (Optional, Computed) Automatic repair node policy. See [`auto_repair_policy`](#management-auto_repair_policy) below.
...
* `auto_upgrade_policy` - (Optional, Computed) The auto update policy. See [`auto_upgrade_policy`](#management-auto_upgrade_policy) below.
* `auto_vul_fix` - (Optional, Computed) Specifies whether to automatically patch CVE vulnerabilities. Valid values: `true`, `false`.
* `auto_vul_fix_policy` - (Optional, Computed) The auto CVE patching policy. See [`auto_vul_fix_policy`](#management-auto_vul_fix_policy) below.
...
* `max_unavailable` - (Optional, Deprecated since v1.216.0) Maximum number of unavailable nodes. Default value: 1. Value range:\[1,1000\].
* `surge` - (Optional, Deprecated since v1.216.0) Number of additional nodes. You have to specify one of surge, surge_percentage.
* `surge_percentage` - (Optional, Deprecated since v1.216.0) Proportion of additional nodes. You have to specify one of surge, surge_percentage.
...
* `max_parallelism` - (Optional) The maximum number of unusable nodes.

### `scaling_config`

The scaling_config supports the following:
* `eip_bandwidth` - (Optional, Deprecated since v1.216.0) Peak EIP bandwidth. Its valid value range [1~500] in Mbps. Default to `5`.
* `eip_internet_charge_type` - (Optional, Deprecated since v1.216.0) The EIP billing type. Default value: 'PayByBandwidth '. Valid values:
  - `PayByBandwidth` : Billing based on fixed bandwidth.
  - `PayByTraffic` : Billing by usage flow.
* `enable` - (Optional) Whether to enable automatic scaling. Value:
  -'true': enables the node pool auto-scaling function.
  -'false': Auto scaling is not enabled. When the value is false, other 'auto_scaling' configuration parameters do not take effect.Default value: 'false '.
* `is_bond_eip` - (Optional, Deprecated since v1.216.0) Whether to bind EIP for an instance. Default: `false`.
* `max_size` - (Optional) Max number of instances in a auto scaling group, its valid value range [0~1000]. `max_size` has to be greater than `min_size`.
* `min_size` - (Optional) Min number of instances in a auto scaling group, its valid value range [0~1000].
