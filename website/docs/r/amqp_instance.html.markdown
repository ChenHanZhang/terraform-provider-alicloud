...
* `auth_model` - (Optional, Available since v1.283.0) Authentication mode. Valid values: ram (RAM mode) and openSource (open-source mode).
...
* `resource_group_id` - (Optional, Computed, Available since v1.283.0) The ID of the resource group to which the resource belongs.
...
* `serverless_switch` - (Optional, Computed, Available since v1.283.0) The toggle status for the elasticity capability of the Serverless instance.
...
* `tags` - (Optional, Map, Available since v1.283.0) List of tags  
...
* `expire_time` - The timestamp when the instance expires, in milliseconds.
* `private_end_point` - The VPC endpoint of the instance.
* `public_endpoint` - Public endpoint of the instance.
