...
* `service_support_ipv6` - (Optional, Computed) Specifies whether the endpoint service supports IPv6. Valid values:
  - `true`: Yes.
  - `false` (default): No.
* `supported_region_list` - (Optional, Computed, List, Available since v1.282.0) The list of regions supported by the endpoint service. Service consumers can initiate endpoint connections from the regions in this list.
