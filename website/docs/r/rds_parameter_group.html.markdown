...
* `engine` - (Required, ForceNew) The database engine. Valid values:
  - `mysql`
  - `PostgreSQL`.
* `engine_version` - (Required, ForceNew) Database engine version. Valid values:  
  - MySQL:  
    * **5.6**  
    * **5.7**  
    * **8.0**  
...
-> **NOTE:**  Adds the parameters provided in the `Parameters` property to the current parameter template, or updates existing parameters in the template. Other parameters in the current template remain unaffected.

* `Individual`: Replace.

-> **NOTE:**  Replaces all parameters in the current parameter template with the parameters provided in the `Parameters` property.


-> **NOTE:** This parameter only applies during resource update. If modified in isolation without other property changes, Terraform will not trigger any action.

* `param_detail` - (Required, List) A list of parameters. See [`param_detail`](#param_detail) below.
* `parameter_group_desc` - (Optional) Description of the parameter template. The length must be 0 to 200 characters.  

-> **NOTE:**  If this parameter is not specified, the original parameter template description remains unchanged.  

* `parameter_group_name` - (Required) Parameter template name.  
* Must start with a letter and can contain letters, digits, periods (.), or underscores (_).  
* Length must be 8 to 64 characters.  

-> **NOTE:**  If this parameter is not specified, the original parameter template name remains unchanged.  

* `resource_group_id` - (Optional, Computed) Resource group ID. You can obtain it by calling the DescribeDBInstanceAttribute operation.  
