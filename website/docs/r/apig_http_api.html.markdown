...
* `ai_protocols` - (Optional, List, Available since v1.282.0) AI protocols
* `base_path` - (Optional) API base path, which must start with a forward slash (/).
* `deploy_configs` - (Optional, List, Available since v1.282.0) API deployment configurations. Currently, only AI APIs support deployment configurations, and only a single deployment configuration can be specified.
* `description` - (Optional) API description.
* `environments` - (Optional, ForceNew, List, Available since v1.282.0) Information about published environments.   See [`environments`](#environments) below.
* `gateway_id` - (Optional, ForceNew, Available since v1.282.0) The ID of the cloud-native API gateway.  
* `gateway_type` - (Optional, ForceNew, Available since v1.282.0) Filter by gateway type. Currently, `AI` and `API` gateway types are supported.
* `http_api_name` - (Required, ForceNew) Perform an exact search by name.
* `model_category` - (Optional, Available since v1.282.0) AI model category
