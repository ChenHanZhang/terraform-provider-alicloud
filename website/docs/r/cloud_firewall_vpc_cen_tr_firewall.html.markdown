...
* `tr_attachment_slave_cidr` - (Optional, ForceNew) The secondary CIDR block of the subnet in the firewall VPC used to connect to TR in automatic mode.
...
* `tr_attachment_id` - The attachment ID used to connect the firewall VPC to the Transit Router (TR) in automatic mode.
