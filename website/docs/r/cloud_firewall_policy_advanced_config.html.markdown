...
* `internet_switch` - (Required) Modifies the switch status of strict mode for access control policies. Valid values:
...
```shell
$ terraform import alicloud_cloud_firewall_policy_advanced_config.example <Alibaba Cloud Account ID>
```