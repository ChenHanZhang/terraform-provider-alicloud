...
* `auto_snapshot_policy_id` - (Optional, ForceNew, Available since v1.274.0) The ID of the automatic snapshot policy that is applied to the cloud disk.
...
* `encrypted` - (Optional, ForceNew, Computed) Specifies whether to encrypt the disk. Default value: `false`. Valid values:
  - `true`: Enable.
  - `false`: Disable.
* `image_id` - (Optional, ForceNew, Available since v1.274.0) The ID of the image to be used to replace the system disk.
...
* `iops` - (Optional, ForceNew, Int, Available since v1.274.0) Number of read/write (I/O) operations per second
* `iops_write` - (Optional, ForceNew, Int, Available since v1.274.0) Number of writes per second
...
* `source_disk_id` - (Required, ForceNew, Available since v1.274.0) SourceDiskId
* `storage_cluster_id` - (Optional, ForceNew, Available since v1.274.0) The ID of the dedicated block storage cluster. If you need to create a cloud disk in the specified dedicated block storage cluster, specify this parameter. For more information about dedicated block storage clusters, see [What is a dedicated block storage cluster](~~ 208883 ~~).
